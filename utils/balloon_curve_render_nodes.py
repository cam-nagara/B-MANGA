...
def _build_nodes(group) -> None:
    ...
    # 塗り面 (FillCurve) は Python 側 (balloon_fill_mesh) で焼き込むため、
    # ここでは生成しない。 body_curve は join しない (= 描画から除外)。
...
def _set_modifier_values(
    ...
    *,
    ...
    thorn_multi_line_valley_width_pct: float = 100.0,
    thorn_multi_line_peak_width_pct: float = 100.0,
    ...
    values = {
        ...
        # 谷/山の線幅 (mm) socket は legacy GN 多重線で使われる絶対 mm 値。
        # pct を base 線幅に掛けて mm 値を渡す。
        "谷の線幅 (mm)": float(multi_line_width_mm or 0.0) * float(thorn_multi_line_valley_width_pct or 0.0) / 100.0,
        "山の線幅 (mm)": float(multi_line_width_mm or 0.0) * float(thorn_multi_line_peak_width_pct or 0.0) / 100.0,
...
def ensure_modifier(
    ...
    *,
    ...
    _set_modifier_values(
        ...
        thorn_multi_line_valley_width_pct=thorn_multi_line_valley_width_pct,
        thorn_multi_line_peak_width_pct=thorn_multi_line_peak_width_pct,
