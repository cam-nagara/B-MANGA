...
Material は per-coma (``BManga_ComaPlane_<page>_<coma>``) に分離。 これにより
...
COMA_PLANE_MATERIAL_PREFIX = "BManga_ComaPlane_"
COMA_PLANE_UV_NAME = "BMangaComaUV"
COMA_PLANE_SOFT_MASK_ATTR = "BMangaComaSoftMask"
...
COMA_MASK_SOLIDIFY_NAME = "BManga Coma Mask Solidify"
...
PROP_COMA_PLANE_KIND = "bmanga_coma_plane_kind"  # "coma_plane"
PROP_COMA_PLANE_OWNER_ID = "bmanga_coma_plane_owner_id"  # "<page_id>:<coma_id>"
PROP_COMA_MASK_KIND = "bmanga_coma_mask_kind"  # "coma_mask"
PROP_COMA_MASK_OWNER_ID = "bmanga_coma_mask_owner_id"

# raster Mesh の Object.location.z (= rank * BMANGA_Z_STEP_M, 1 段目で 0.01)
...
# 2026-05-04: BMANGA_Z_STEP_M を 0.1 → 0.01 に縮小したのに合わせて 0.1 → 0.01
# に変更。
...
def _existing_material_image(mat: bpy.types.Material):
    ...
    for node in nt.nodes:
        if (
            ...
            and node.name != "BManga_ComaAlphaMask"
...
def _resolve_preview_image(work, page, coma):
    ...
    if img is None:
        ...
        try:
            if float(img.get("_bmanga_mtime", -1.0)) != mtime:
                img.reload()
        except Exception:  # noqa: BLE001
            pass
    try:
        img["_bmanga_mtime"] = mtime
...
_UV_ANCHOR_MIN_SIZE_M = 0.005  # 5mm 未満の bbox はフォールバック状態とみなしてアンカーに採用しない


def _ensure_uv(mesh: bpy.types.Mesh, coma=None) -> None:
    """コマ形状メッシュに UV を割り当てる.

    UV のアンカー (基準矩形) は ``mesh["bmanga_uv_ref"]`` に保存され、
    一度ロックすると以降のジオメトリ変更では再計算しない (プレビュー画像が
    コマ枠拡張に追従して変形しないようにするため)。 アンカーは
    ``_refresh_uv_anchor_for_image`` が画像 mtime の更新を検知した時にのみ
    現在のコマデータの外接矩形へ張り替える。

    フォールバック状態 (頂点が一時的に空で 1mm の三角形が置かれている等) を
    検知して採用しないため、 ``coma`` を渡せばコマデータから真の bbox を
    取得する。 渡されなかった場合はメッシュの bbox を使うが、 ``5mm`` 未満
    なら採用しない。
    """
    if mesh is None or len(mesh.vertices) == 0 or len(mesh.loops) == 0:
        return
    ref = _get_uv_anchor(mesh)
    if ref is None:
        ref = _bbox_from_coma_m(coma) or _bbox_from_mesh(mesh)
    if ref is None:
        return
    min_x, min_y, w, h = ref
    if w <= 0.0 or h <= 0.0:
        return
    # 初回 (アンカー未保存) かつ採用に値する大きさなら保存
    if not _has_valid_uv_anchor(mesh) and _is_reasonable_bbox_m(w, h):
        mesh["bmanga_uv_ref"] = [min_x, min_y, w, h]
...
def _fit_preview_uv(mesh: bpy.types.Mesh, u: float, v: float, w: float, h: float) -> tuple[float, float]:
    mode = str(mesh.get("bmanga_uv_fit_mode", "") or "")
    if mode != "cover":
        return u, v
    try:
        image_aspect = float(mesh.get("bmanga_uv_image_aspect", 0.0) or 0.0)
...
def _is_reasonable_bbox_m(w: float, h: float) -> bool:
    return w >= _UV_ANCHOR_MIN_SIZE_M and h >= _UV_ANCHOR_MIN_SIZE_M


def _has_valid_uv_anchor(mesh: bpy.types.Mesh) -> bool:
    ref = _get_uv_anchor(mesh)
    if ref is None:
        return False
    _x, _y, w, h = ref
    return _is_reasonable_bbox_m(w, h)


def _get_uv_anchor(mesh: bpy.types.Mesh) -> tuple[float, float, float, float] | None:
    raw = mesh.get("bmanga_uv_ref") if mesh is not None else None
    if not raw:
        return None
    try:
...
def _bbox_from_coma_m(coma) -> tuple[float, float, float, float] | None:
    ...
    if shape == "rect":
        ...
        if w_mm <= 0.0 or h_mm <= 0.0:
            return None
        # rect の mesh ローカル原点は (0, 0) で頂点が (0..w, 0..h) に展開される
        return (mm_to_m(0.0), mm_to_m(0.0), mm_to_m(w_mm), mm_to_m(h_mm))
    verts = getattr(coma, "vertices", None) or []
    if len(verts) < 3:
        return None
    pts = [(float(v.x_mm), float(v.y_mm)) for v in verts]
    xs = [p[0] for p in pts]
    ys = [p[1] for p in pts]
    min_x, max_x = min(xs), max(xs)
    min_y, max_y = min(ys), max(ys)
    ...
    if w <= 0.0 or h <= 0.0:
        return None
    # メッシュは頂点を原点センター化していないので min_x/min_y もそのまま採用
...
def _refresh_uv_anchor_for_image(mesh: bpy.types.Mesh, image, coma=None, work=None) -> None:
    ...
    if mesh is None or image is None or len(mesh.vertices) == 0:
        return
    try:
        img_mtime = float(image.get("_bmanga_mtime", 0.0) or 0.0)
    except (TypeError, ValueError):
        return
    if img_mtime <= 0.0:
        return
    try:
        stored_mtime = float(mesh.get("bmanga_uv_ref_mtime", -1.0) or -1.0)
    except (TypeError, ValueError):
        stored_mtime = -1.0
    ...
    fit_mode_stale = str(mesh.get("bmanga_uv_fit_mode", "") or "") != expected_fit_mode
    ...
    if use_cover_fit:
        ...
        try:
            mesh["bmanga_uv_fit_mode"] = "cover"
            mesh["bmanga_uv_image_aspect"] = _image_aspect(image)
        except Exception:  # noqa: BLE001
            pass
    else:
        bbox = _page_anchor_m(work, coma) or _bbox_from_coma_m(coma) or _bbox_from_mesh(mesh)
        try:
            mesh["bmanga_uv_fit_mode"] = "page"
            mesh["bmanga_uv_image_aspect"] = 0.0
        except Exception:  # noqa: BLE001
            pass
    if bbox is None:
        return
    _x, _y, w, h = bbox
    if not _is_reasonable_bbox_m(w, h):
        # フォールバック (1mm 三角形 等) のまま採用しない
        return
    mesh["bmanga_uv_ref"] = list(bbox)
    mesh["bmanga_uv_ref_mtime"] = img_mtime
...
def _apply_material(
    ...
    *,
    ...
    def _alpha_value_socket(value: float):
        ...
        node.name = "BManga_ComaAlphaValue"
    ...
    def _link_alpha_surface(alpha_socket) -> None:
        ...
        transparent.name = "BManga_ComaTransparent"
        transparent.location = (90, -170)
        mix = nt.nodes.new("ShaderNodeMixShader")
        mix.name = "BManga_ComaAlphaMix"
        mix.location = (250, -30)
        try:
    ...
    if use_soft_mask:
        ...
        alpha_attr.name = "BManga_ComaSoftMask"
        ...
        if dither:
            ...
            compare.name = "BManga_ComaDitherThreshold"
            compare.label = "ディザ判定"
        ...
        if image is not None:
            alpha_factor = _multiply_alpha(
                tex.outputs["Alpha"],
                edge_factor,
                name="BManga_ComaPreviewAlphaMask",
                y=-120,
            )
        elif a < 0.9999:
            alpha_factor = _multiply_alpha(
                ...
                edge_factor,
                name="BManga_ComaColorAlphaMask",
                y=-120,
            )
...
def _create_dither_value_nodes(nt: bpy.types.NodeTree):
    ...
    try:
        ...
        noise.name = "BManga_ComaDither"
        noise.label = "ディザ"
        noise.location = (-300, -470)
        texcoord = nt.nodes.new("ShaderNodeTexCoord")
        created.append(texcoord)
        texcoord.name = "BManga_ComaDitherCoord"
        texcoord.location = (-760, -470)
        mapping = nt.nodes.new("ShaderNodeMapping")
        created.append(mapping)
        mapping.name = "BManga_ComaDitherScale"
...
def _ensure_coma_plane_material(
    ...
    # 再レンダリング時には新しい画像へ正しく追従させる)。 メッシュが一時的に
    # フォールバック状態 (1mm 三角形) でも、 コマデータ側の bbox を使うため
    # 誤ったアンカー固定は起きない。
    try:
...
def ensure_coma_mask(
    ...
    obj[on.PROP_MANAGED] = False  # B-MANGA Outliner mirror 対象外
    ...
    coma_coll = on.find_collection_by_bmanga_id(owner_id, kind="coma")
...
def _find_page_and_coma_for_vertex(work, target_vertex):
    """``BMangaComaVertex`` の所属コマ・ページを ``as_pointer()`` で探す."""
...
def on_vertex_changed(vertex) -> None:
    """``BMangaComaVertex.x_mm`` / ``y_mm`` 変更時に呼ぶ.
