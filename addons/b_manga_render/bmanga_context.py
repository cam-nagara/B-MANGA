"""Read-only helpers for B-MANGA coma blend context."""
...
class BMangaComaContext:
    ...
    @property
    def is_bmanga_coma(self) -> bool:
        return bool(self.page_id and self.coma_id)


def scene_context(scene) -> BMangaComaContext:
    ...
    return BMangaComaContext(page_id, coma_id, blend_path, passes_dir, images)
...
def default_output_name(scene, requested: str = "", preset_name: str = "") -> str:
    requested = str(requested or "").strip()
    context = scene_context(scene)
    if requested and not context.is_bmanga_coma:
        return requested
    if requested and context.is_bmanga_coma and requested not in LEGACY_DEFAULT_OUTPUT_NAMES:
...
def _managed_page_image_names(scene) -> list[str]:
    ...
    for bg in getattr(camera_data, "background_images", []) or []:
        ...
        try:
            ...
            is_page = bool(image.get("bmanga_full_page_mask", False)) or str(image.get("bmanga_kind", "") or "") == "name"
