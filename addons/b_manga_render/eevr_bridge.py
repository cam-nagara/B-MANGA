...
The original eeVR addon is GPL-3.0. B-MANGA Render keeps this bridge small and
...
def setup(scene, camera=None, *, output_dir: str = "", output_name: str = "") -> bool:
    ...
    stored_dir = bmanga_context.default_output_folder(
        scene,
        output_dir or str(scene.get("bmanga_render_fisheye_output_dir", "") or ""),
    )
    stored_name = bmanga_context.default_output_name(
        scene,
        output_name or str(scene.get("bmanga_render_fisheye_output_name", "") or ""),
    )
    projection = bmanga_context.camera_fisheye_projection(camera)
    dome_method = bmanga_context.eevr_dome_method_for_projection(projection)
    if props is not None:
        if not dome_method:
            if bmanga_context.scene_context(scene).is_bmanga_coma:
                scene["bmanga_render_fisheye_warning"] = "対応する魚眼投影方式が見つかりません"
                return False
            dome_method = "2"
    ...
    scene["bmanga_render_fisheye_output_dir"] = stored_dir
    scene["bmanga_render_fisheye_output_name"] = stored_name
    return True
...
def _fisheye_fov(scene, camera) -> float:
    ...
    if bool(getattr(scene, "bmanga_coma_camera_fisheye_layout_mode", False)):
        value = getattr(scene, "bmanga_coma_camera_fisheye_fov", value)
    if value is None:
...
def _output_base(scene, suffix: str) -> str:
    folder = bmanga_context.default_output_folder(
        scene,
        str(scene.get("bmanga_render_fisheye_output_dir", "") or ""),
    )
    name = bmanga_context.default_output_name(
        scene,
        str(scene.get("bmanga_render_fisheye_output_name", "") or ""),
    )
