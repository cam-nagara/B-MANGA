...
from . import object_rotation_image_path  # noqa: F401 (import時にimage_path回転ハンドラーを登録)

_logger = log.get_logger(__name__)

TOOL_NAME = "image_path_tool"
_MIN_POINT_DIST_PX = 2.0
_MIN_POINTS = 2
_SIMPLIFY_TOLERANCE_MM = 0.3
_COLOR_LINE = (0.1, 0.55, 0.95, 0.95)
...
def _draw_callback(op: "BMANGA_OT_image_path_tool") -> None:
    pts = op._points_px
...
class BMANGA_OT_image_path_tool(Operator):
    ...
    def modal(self, context, event):
        ...
        # 選択ハンドル回転リングのホバーカーソル (ストローク描画中は対象外)
        ...
        if ev_type == "LEFTMOUSE" and ev_value == "RELEASE" and self._points_px:
            return self._on_release(context)
    ...
    def _on_press(self, context, event):
        ...
        if result is None:
            return {"RUNNING_MODAL"}
        _area, region, _rv3d, mx, my, wx, wy = result
        if not self._lock_page(context, wx, wy):
            return {"RUNNING_MODAL"}
        self._points_px = [(mx, my)]
        self._points_mm = [(wx - self._page_ox, wy - self._page_oy)]
        self._region = region
        return {"RUNNING_MODAL"}
    ...
    def _lock_page(self, context, world_x_mm, world_y_mm) -> bool:
        ...
        self._coma_key = coma_stack_key(page, coma) if coma is not None else ""
        return True

    def _create_image_path(self, context, points_mm: list) -> None:
        ...
        try:
            ...
            work = get_work(context)
            page = image_path_object.page_for_entry(context.scene, work, entry)
            ...
            _logger.exception("image path object creation failed")
...
_CLASSES = (BMANGA_OT_image_path_tool,)
