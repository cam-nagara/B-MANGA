...
- **(進行中 / 2026-07-14 Codex)** 詳細設定ダイアログ一元化。レイヤー一覧、3Dビュー／アウトライナー右クリック、プリセット歯車の描画・幅・対象解決・キャンセルを共通化する。先行条件として、GP／旧集約効果線／リンクUIDを全ページ用blendファイルで安全に移行する基盤も実装する。実作品の変換は対象ページ数・退避先・必要容量を提示してユーザー確認を得るまで実行しない。計画: [`docs/detail_dialog_unification_plan_2026-07-14.md`](docs/detail_dialog_unification_plan_2026-07-14.md)
