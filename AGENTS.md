...
**bump 対象 (両方更新)**:
- `blender_manifest.toml` の ``version = "X.Y.Z"`` (これが authoritative)
- `__init__.py` の ``bl_info["version"] = (X, Y, Z)`` (legacy 互換)

**SemVer 簡易ルール** (本プロジェクトは安定 1.0.0 前なので 0.Y.Z 帯):
- **PATCH (Z)**: ユーザー操作の挙動修正 / バグ修正 / UI 微調整 / 既存機能の小拡張
- **MINOR (Y)**: 新機能 / アーキテクチャ変更 / データ構造変更 / 削除/リネームを伴う API 変更
- **MAJOR (X)**: 1.0.0 突入。 当面なし

**PATCH 桁運用**:
- PATCH (Z) は 0〜999 の三桁範囲で運用する。例: `0.5.999` の次は `0.6.0` とし、 `0.5.1000` にはしない。
...
直近の事例 (参考): v0.1.7 コマ用 blend 設定改善 → v0.1.8 フキダシ Outliner 同期 → v0.1.9 コマプレビュー修正 → v0.2.0 効果線 Object 化 (アーキ変更で MINOR bump) → v0.2.1 mask_layers API シグネチャ修正 (PATCH) → v0.2.2 ツールセクション再構成 + 詳細設定復元 (PATCH)。
