...
- **Blender: 5.2 LTS** (2026-07-14リリース)。 API は 5.2 LTS 仕様で確認すること。 Windows での実行ファイルは `C:\Program Files\Blender Foundation\Blender 5.2\blender.exe`
...
- **(Phase 1〜4実装済み／ユーザー実機合否待ち / 2026-07-28 Codex, v0.6.592)** ページ編集用の2D合成プレビューを試験導入。PNG書き出しと同じレイヤー順で全体をGPUオーバーレイ表示し、GP／効果線／ラスター編集中は背面合成・編集実体・前面合成へ分割する。72dpi以下を即時表示し150ms後に設定解像度へ更新、512MiB LRU、未保存ラスター反映、保存時の一時Image除去、Undo／Redo再合成を実装。レイヤー移動／オブジェクト移動中は切り出したGPU画像だけを動かし、確定時にデータを一括反映する。従来3D表示への切替を残し、ユーザー実機確認が通るまでは「2D合成表示（試験）」を既定オフとする。

- **(完了 / 2026-07-17 Codex, v0.6.530)** Meldexシナリオ取込を作品単位の一括保存・復元へ変更。ファイル読込と直接送信の両方で、既存ページの先読込、Dropbox／別画面競合の事前拒否、対象JSON全件とメモリ状態の失敗復元、強制終了後の自動復元、新規空フォルダー清掃、フキダシIDカウンター永続化を共通化した。

- **(完了 / 2026-07-17 Codex, v0.6.528)** Meldex本文・ルビ表示設定の任意取込。プリファレンスへ初期オフの「Meldexのテキスト設定を適用」を追加し、オフではB-MANGAテキストプリセット、オンではその上へMeldexの本文・ルビ共通表示設定を適用する。ファイル読込と直接送信、文書全体と行別上書き、論理フォント解決、CSS単位・色空間変換を統一した。

- **(完了 / 2026-07-17 Codex, v0.6.527)** Meldexシナリオ取込の表示設定は、ファイル読込・直接送信ともB-MANGAのテキストプリセットを常に正本とするよう変更した。「Meldexのルビ表示設定を適用」は設定値とUIを廃止し、旧保存データにオン値が残っていても適用しない。本文、ルビ範囲、読み、明示されたルビ種類・由来・優先順位・熟語内訳は従来どおり保持する。

- **(完了 / 2026-07-16 Codex, v0.6.526)** 旧Meldexシナリオの3.5px互換距離を書字方向別の旧描画座標から共通の相対間隔へ変換し、ファイル読込と直接送信を同一化。縦書きルビにも本文と同じ括弧回転、句読点・小書き位置補正、英数字正立を適用した。指定実シナリオと縦横6画面の記号・濁音／半濁音・全小書き仮名をBlender 5.1.2で自動検査・AI目視済み。
...
- **(完了 / 2026-07-11 Codex, B-MANGA Liner v0.3.198)** 三角面・重複面・ゼロ面積面・孤立要素・閉じた純形状の非多様体を候補上で修復し、UV・素材・分割法線・開口輪郭・手動クリースを保ったCatmull-Clark向け全面四角形へ一括確定する「問題メッシュ自動修復・四角面化」を追加。健全な多角形を維持して一直線上の頂点を安全に四角面化し、大規模三角面素材は元面対応による直接属性転送で高速化した。危険な対象は全件未変更で中止する。詳細: [`docs/bml_auto_quad_repair_plan_2026-07-11.md`](docs/bml_auto_quad_repair_plan_2026-07-11.md)
...
- Claude Code: 実行中のモデル表示名を使う (例: `Co-Authored-By: Claude Fable 5 <noreply@anthropic.com>`。特定バージョン名をこの規約に固定しない — グローバル AGENTS.md 準拠)
...
- [ ] **§1.1 に従いユーザー実機 (`extensions\vscode_development\b_manga`) へ配備し、チャットで Blender 再起動を依頼した** (コミットだけでは実機に反映されない。2026-07-22 に配備漏れで「修正したのに直っていない」が実際に発生)
...
| 詳細設定の3入口・固定対象・キャンセル | `test/blender_detail_dialog_unification_check.py` / `test/blender_detail_effect_fixed_target_check.py` |
| 詳細設定の最大幅・等幅列（通常画面） | `test/blender_detail_dialog_width_visual_check.py` |
| 対象を隠さない確認ダイアログ（通常画面） | `test/blender_target_confirm_visual_check.py` |
| 選択文字設定の右側配置（通常画面） | `test/blender_text_selection_popup_visual_check.py` |
| 詳細設定の未保存プリセット切替確認（通常画面） | `test/blender_detail_preset_unsaved_confirm_visual_check.py` |
| 詳細設定後のオブジェクトツール再開（通常画面） | `test/blender_detail_dialog_object_tool_resume_check.py` |
| リンクテキスト余白・フキダシプリセット保存／反映 | `test/blender_linked_balloon_preset_padding_check.py` |
| 旧作品の全ページ変換・復元 | `test/blender_detail_dialog_data_migration_check.py` / `test/blender_detail_migration_open_file_reload_check.py` |
| ページ複製・削除の内容保持 | `test/blender_page_duplicate_delete_transaction_check.py` |
| ページ／コマファイルの現在ページ枠 | `test/blender_current_page_outline_visual_check.py` |
| ページプレビューのフキダシ内テキスト太さ | `test/blender_page_preview_text_weight_check.py` |
| 見開き結合・分割の内容保持 | `test/blender_spread_content_roundtrip_check.py` |
...
| Meldexシナリオファイル読込 | `test/blender_meldex_scenario_file_import_check.py` / `test/blender_meldex_scenario_file_panel_visual_check.py` |
| ダブルクリック開きとキーマップ再構築の競合（通常画面） | `test/blender_double_click_keymap_crash_guard_ui_check.py` |
| 作品ファイルでのレイヤー選択ガード（通常画面） | `test/blender_work_list_layer_pick_guard_check.py` |
| 作品「開く」のファイルフィルターOFF（通常画面） | `test/blender_work_open_file_filter_visual_check.py` |
| Meldexシナリオ取込の一括保存・失敗復元 | `test/blender_meldex_scenario_import_transaction_check.py` |
| ルビ内記号・濁音／半濁音・小書き（縦横実描画） | `test/blender_ruby_symbols_visual_check.py` |
| 縦記号の中央揃え・本文／ルビ共通フチ（複数フォント実描画） | `test/blender_text_fringe_symbol_visual_check.py` |
...
| Undo/Redoの微小編集・ファイル境界 | `test/blender_undo_redo_runtime_check.py` |
| restructure E2E | `test/blender_restructure_e2e.py` |

実行例 (Windows / bash):

```bash
'/c/Program Files/Blender Foundation/Blender 5.2/blender.exe' --background --factory-startup --python-exit-code 1 --python test/blender_xxx_check.py
```
...
- B-MANGAのデータや表示実体を変更する新規操作は、実装時にUndo境界を必ず分類する。単発Operatorは原則 `bl_options` の `UNDO`、モーダル操作は操作開始時の値スナップショットと確定時の最終値を比較し、実変更がある場合だけ `utils/undo_transaction.py` 経由で1回積む。
- 「ドラッグと見なす距離」「クリック誤操作を防ぐ距離」等のUIしきい値を、Undo対象かどうかの判定へ流用しない。微小変更も履歴化し、開始値へ戻した操作とクリックだけの操作は空履歴にしない。
- `bpy.ops.ed.undo_push` を各機能へ直接追加しない。Undo/Redoのpre/post中はObjectから作品データへの監視書戻しを止め、post再同期からJSON・PNG保存や新しいUndoステップを発生させない。
- 作品／ページ／コマのファイル切替は履歴境界として扱う。ファイル作成、外部JSON・PNG・PSD保存、ページ間のファイル取引はBlender Undoへ混ぜず、既存の保存トランザクションと失敗復元で保護する。
- 新機能・仕様変更・機能削除では、通常変更、微小変更、開始値へ戻す操作、Undo、Redo、保存再読込を対象機能の決定的テストへ追加する。Undo契約の静的テストを変更して黙って除外を増やさない。
...
- 自動メモリ: `C:\Users\niken\.claude\projects\D--Develop-Blender-B-MANGA\memory\` に永続化
