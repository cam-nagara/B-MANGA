...
- **(Phase 0判断待ち / 2026-07-14 Codex)** 詳細設定ダイアログ一元化。Blender 5.1.2実機で、編集中の値とキャンセル境界を維持した安全な動的幅変更が成立しないことを確認した。計画書の停止条件に従い実行コードは未変更で、種別ごとの最大幅固定（推奨）／1列化／専用画面化のユーザー判断待ち。GP／旧集約効果線／リンクUIDの実作品変換も未実行。計画: [`docs/detail_dialog_unification_plan_2026-07-14.md`](docs/detail_dialog_unification_plan_2026-07-14.md)
