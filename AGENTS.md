...
- **(進行中 / 2026-07-14 Codex)** 詳細設定ダイアログ一元化。Blender 5.1.2では安全な動的幅変更が成立しないため、ユーザー判断で種別ごとの安全な最大幅固定を採用した。レイヤー一覧、3Dビュー／アウトライナー右クリック、プリセット歯車の描画・対象解決・キャンセルを共通化し、先行してGP／旧集約効果線／リンクUIDの全ページ移行基盤を実装する。実作品の変換は対象ページ数・退避先・必要容量を提示してユーザー確認を得るまで実行しない。計画: [`docs/detail_dialog_unification_plan_2026-07-14.md`](docs/detail_dialog_unification_plan_2026-07-14.md)
