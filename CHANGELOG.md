...
### 症状 / ご要望
1. ビューセクションに「選択ページを直接切り替える数値入力」がなく、 アクティブ
   ページの行き来に都度ボタン操作が必要だった。
2. 紙面上で **コマ外+ページ内** をクリックしても何も選択されなかった (ページが
   選択不能)。 さらに、 そのときに効果線 / フキダシ / テキストツールでドラッグ
   作成すると、 アウトライナー上のページコレクションでコマよりも下に並び、
   重なり順もコマに被ってしまった。
3. ラスター描画中に ``Ctrl+Alt+ドラッグ`` を押してもブラシサイズが変えられず、
   グリースペンシル描画と挙動が違っていた。
4. アウトライナーで Alt+ドラッグでページ外にドロップしても何も視覚的フィード
   バックがなく、 「いまどこに行こうとしているか」 がわからなかった。
5. ツールセクションのグリースペンシル / ラスター切替ボタンがレイヤー種別に
   応じて出たり消えたりし、 「即座に切替えられない」 状態だった。

### 修正
- ``panels/view_panel.py``: ビューセクションに 「選択ページ」 の前後ボタン+
  数値入力欄を追加。 アクティブページを直接指定できる。
- ``operators/object_tool_op.py``: 紙面のヒット判定にページ枠 (コマ外+ページ内)
  を追加。 該当箇所をクリックすると、 そのページがアクティブになりコマ選択は
  解除される (ドラッグでページごと動かす挙動はしない)。
- ``utils/object_naming.py`` + ``utils/layer_object_sync.py``: parent_kind が
  ``"page"`` のとき、 レイヤー Object 名 prefix を ``L`` ではなく ``A`` にする
  ヘルパ ``prefix_char_for(kind, parent_kind)`` を追加。 アウトライナーが
  alpha sort のとき、 ページコレクション内でコマ (``c01`` など) より上端に
  並ぶようにした。
- ``operators/effect_line_op.py`` / ``utils/balloon_curve_object.py`` /
  ``utils/empty_layer_object.py``: 効果線 / フキダシ / テキストの z_index ベース
  値を、 parent_kind が ``"page"`` のとき高め (5000 / 6000 / 7000) に切替えて、
  同ページのコマ配下レイヤーより常に前面で描画されるようにした。
- ``operators/brush_size_op.py``: ブラシサイズドラッグの対象を GP 系に加えて
  TEXTURE_PAINT の ``image_paint`` ブラシまで拡張。 ラスター描画中の
  ``Ctrl+Alt+ドラッグ`` でも GP と同じ操作感でサイズ調整できる。
- ``keymap/keymap.py``: Image Paint キーマップに ``bname.brush_size_drag``
  (Ctrl+Alt+LMB) を登録し、 TEXTURE_PAINT 中も先取り。
- ``ui/reparent_overlay.py`` + ``operators/alt_reparent_op.py``: Alt+ドラッグ /
  Alt+クリック / Alt+Shift+クリックでページ外ドロップ位置に円+斜め十字の朱色
  インジケーターを表示。 ページ / コマターゲットと同じく、 確定演出も短時間
  フラッシュで描画される。
- ``panels/tool_panel.py`` + ``panels/gpencil_panel.py``: グリースペンシル描画
  / ラスター描画 / 線編集アイコンを **常時併設** に変更。 アクティブレイヤー
  種別に関わらず即座に切替できる。 GP 描画ボタンは現在アクティブレイヤーが
  GP でないときも、 マスター GP を自動アクティブ化してフォールバックする。

### 検証 (Blender 5.1.1 実機予定)
- ビューセクションのページ番号入力でページが切替わる
- コマ外+ページ内クリックでページ選択 / コマ選択クリアになる
- 効果線 / フキダシ / テキストツールでコマ外+ページ内ドラッグ作成 → アウトライナー
  でコマより上端に並び、 重なり順もコマより前面
- ラスター描画中の Ctrl+Alt+ドラッグでブラシサイズが変わる
- Alt+ドラッグでページ外位置にドロップ時、 朱色インジケーターが追従表示
- ツールセクションのアイコンが常時クリック可能 (グレーアウトしない)
