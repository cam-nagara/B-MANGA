...
B-MANGA の N パネル内 `UIList` で Photoshop / CLIP STUDIO PAINT 風の D&D レイヤー操作を再現する方針を見直し、Blender 標準 Outliner の D&D を利用できる構造へ移行する。

方針は次の通り。

- B-MANGA の論理レイヤーを、Blender の実 `Object` として表現する。
- ページ、コマ、汎用フォルダを、Blender の `Collection` として表現する。
- Outliner 上の Object/Collection D&D を、B-MANGA の親子付け変更 UI として使う。
- B-MANGA パネル上のレイヤーリストは廃止または補助表示へ縮小し、作成、削除、前面/背面移動、詳細編集、整合性修復のボタンを配置する。
- 重なり順は B-MANGA の `z_index` を正とし、`Object.location.z` と Object 名 prefix を自動同期する。
...
Outliner は Object / Collection の所属変更や親子付け D&D を持つ。ただし Python アドオンが Outliner 内へ独自項目を差し込む API ではないため、B-MANGA 独自 `PropertyGroup` のままでは利用できない。
...
| 種別 | 現状 | Outliner への露出 |
|------|------|------------------|
| ページ | `BMangaPageEntry` + `page_pNNNN` Collection | 一部あり |
| コマ | `BMangaComaEntry` | Collection ではない |
| 汎用レイヤーフォルダ | `BMangaLayerFolder` | なし |
| GP | 単一 `bmanga_master_sketch` Object 内の GP レイヤー | 個別レイヤーとしては出ない |
| ラスター | `BMangaRasterLayer` + `raster_plane_*` Object | あり |
| 画像 | `BMangaImageLayer` + GPU overlay 描画 | なし |
| フキダシ | `BMangaBalloonEntry` + GPU overlay 描画 | なし |
| テキスト | `BMangaTextEntry` + GPU overlay / typography 描画 | なし |
...
| B-MANGA論理要素 | Blender実体 |
|----------------|-------------|
| 作品ルート | Collection `B-MANGA` |
...
Object/Collection 名はユーザーが変更できるため、B-MANGA の安定 ID は custom property に保存する。

```text
object["bmanga_kind"] = "image" | "raster" | "gp" | "balloon" | "text" | "effect"
object["bmanga_id"] = "image_0001"
object["bmanga_parent_key"] = "p0001:c01"
object["bmanga_folder_id"] = "folder_xxx"
object["bmanga_z_index"] = 40
object["bmanga_title"] = "セリフ本文"
object["bmanga_managed"] = True
```

#### 既存 PropertyGroup の参照ルール

既存実装に沿って、各エントリの真の安定 ID は次のとおり。計画書全体の `bmanga_id` / `bmanga_parent_key` はこれを写し取った派生値である。

| エントリ | 真の安定 ID プロパティ | 値の例 | 補足 |
|---------|----------------------|-------|------|
| `BMangaPageEntry` | `id` | `p0001`, `p0001-0002` (見開き) | `core/page.py:51`。属性名は `page_id` ではなく `id` |
| `BMangaComaEntry` | `id` | `c01` (`cNN`) | `core/coma.py:87`。`id` を一次キーとする |
| `BMangaComaEntry.coma_id` | (補助) | `c01` (ファイル stem) | `core/coma.py:96`。export 用ファイル名生成に使う**派生値**であり、安定 ID として使ってはならない |
| `BMangaLayerFolder` | `id` | `folder_xxxxxx` | `core/layer_folder.py:16` |
| `BMangaRasterLayer` / `BMangaImageLayer` | `id` | `raster_0001` 等 | `core/raster_layer.py` / `core/image_layer.py` |
| `BMangaBalloonEntry` / `BMangaTextEntry` | `id` | `balloon_0001` 等 | `core/balloon.py` / `core/text_entry.py` |

`object["bmanga_id"]` には上表「真の安定 ID」をそのまま格納する。`object["bmanga_parent_key"]` は既存 `parent_key` 形式 `"<page_id>:<coma_id>"` をそのまま採用し、新フィールド名を作らない（§7 参照）。
...
prefix の数値 4 桁は z_index を 0 詰め 4 桁で表記する（`L0040` 等）。同一階層に同 z_index が衝突した場合は、`bmanga_id` の昇順で内部優先度を決め、prefix の生成は重複しないように `_a` `_b` などのサフィックスで衝突回避する。
...
ユーザーが名前を編集した場合は、prefix 以降をタイトルとして解釈し、prefix は B-MANGA が再生成する。
...
- 安全に扱うため、Object 名生成側で次を実装する。
  1. `bmanga_title` は **常にフルテキストを custom property に保持** し、Object 名側を真とみなさない。
  ...
  3. 切詰め発生時は Object に `bmanga_title_truncated = True` を立て、修復ボタンから検知できるようにする。
- Blender は同名衝突時に `.001` `.002` を自動付加するが、`bmanga_id` を真の安定 ID とするため、検出 scan は **Object 名ではなく `bmanga_id` で B-MANGA エントリと突合**する (`.001` 付き Object も同 `bmanga_id` であれば同一エントリとして扱う)。Object 名 prefix の再生成は必要に応じて B-MANGA 側から `obj.name = canonical` で上書きする。

### 3.3 GP レイヤー

GP は Blender の Grease Pencil 内部レイヤーではなく、**1 つの GP Object を B-MANGA の 1 レイヤーとして扱う**。
...
L0031__gp__髪  (Grease Pencil Object)
  内部 GP layer: content
  内部 GP layer: __bmanga_mask  (必要時のみ、通常は非表示)
```
...
- 既存 `bmanga_master_sketch` からの移行処理が必要。
...
```text
z = z_index * BMANGA_Z_STEP_M
```

初期案:

```text
BMANGA_Z_STEP_M = 0.0001  # 0.1mm
```
...
Outliner のアルファベット順ソートを使うと、prefix によってレイヤー順と表示順を概ね一致させられる。B-MANGA 用 Outliner を開く operator で `display_mode="VIEW_LAYER"`、`use_sort_alpha=True` などを設定する。
...
`BMangaComaEntry.rect_x_mm/rect_y_mm/vertices` は **ページローカル座標 (mm)**。コマを別ページへ D&D した場合、移動先の用紙サイズ・grid offset が異なれば座標が破綻するため、**Phase 0-2 では Coma → Page 跨ぎ移動は拒否**し、`§5.3 正規化` で元のページ Collection へ戻す。

将来 Phase 4 以降で許可する場合は、次のいずれかを必須とする。

1. **明示 operator**: B-MANGA パネルから「コマを別ページへ移動」を選択させ、座標再計算 (`new_rect = old_rect - old_page_offset + new_page_offset`) と vertices 再射影を伴うフローのみ許可する。
...
- `bpy.msgbus`
- 低頻度 timer scan
- B-MANGA 管理 Collection/Object の custom property 差分比較

検出後に、Object の所属 Collection から B-MANGA の `parent_key` / `folder_key` / `z_index` を更新する。

### 5.3 正規化

Outliner 操作後は必ず正規化を行う。

- B-MANGA 管理 Object (`bmanga_managed = True`) は B-MANGA 管理 Collection のどれか 1 つだけに所属させる。
- 複数 Collection に link された場合は、**`bmanga_managed = True` のもののみ自動正規化対象**とし、最後に検出した B-MANGA Collection を採用してそれ以外から unlink する。`bmanga_managed` が無い (ユーザーが手動で Ctrl-D&D で意図的に多重 link した非管理 Object) は触らない。
- ユーザーが B-MANGA 管理 Object を **意図的に多重 link したい** ときの逃げ道として、Object に `bmanga_no_normalize = True` を立てる退避フラグを用意し、検出 scan で skip する。
- 無効な場所へ移動された場合は、直前の有効親へ戻す。Coma Collection の Page 跨ぎは §5.1 のルールで戻す。
- B-MANGA 管理 Object/Collection が削除された場合は、B-MANGA パネルに修復ボタンを出す。自動復元はユーザー操作との競合があるため、初期段階では警告優先にする。
...
1. **モジュールスコープ guard**: `_BMANGA_SYNC_IN_PROGRESS = False` をフラグとし、コールバック先頭で True なら早期 return、自身の処理範囲では set/unset を `try/finally` で囲う。
...
3. **B-MANGA 操作中の suppress**: B-MANGA の operator (前面/背面、reparent 等) は実行ブロック全体を guard で囲い、operator 内変更による再 fire を完全に抑止する。
...
マスクを実現する場合は、B-MANGA が以下を生成・同期する必要がある。
...
- ページマスク用と コママスク用で **別レイヤーに分離する**。
  - `__bmanga_page_mask`: ページ矩形マスク。GP Object がページ Collection 直下にある場合のみ有効。
  - `__bmanga_coma_mask`: コマ形状マスク。GP Object がコマ Collection 内にある場合のみ有効。両方が成立するときは `__bmanga_coma_mask` が `__bmanga_page_mask` を上書きする (内側が必ずページ内のため)。
- 名前 prefix `__bmanga_` の layer は B-MANGA 管理であり、UI には出さない。マスク生成 / 削除 / 形状更新は親 Collection 変化や `BMangaComaEntry.vertices` 更新に追従させる。
...
マスク形状の正は B-MANGA のページ/コマデータとする。

- ページ: `paper.canvas_width_mm` / `paper.canvas_height_mm`
- コマ: `BMangaComaEntry.shape_type` / `vertices` / `rect_*`
...
- `objectName` は派生表示。次回読込時に `bmanga_id` から prefix を再生成し、`objectName` フィールドは検証用情報としてのみ使う (.001 自動付加検出用)。
...
目的: Outliner を B-MANGA レイヤー管理 UI として使えるかを最小検証する。

作業:

1. B-MANGA root / outside / page / coma / folder Collection を生成する試作。
...
6. B-MANGA 用 Outliner 表示の取り扱いを確定する。**既存 Outliner エディタの設定を上書きしない方針**を採用し、次のいずれかを選ぶ:
   - **(a)** 専用 workspace `B-MANGA` を提供する。`B-MANGA` workspace の Outliner だけ `display_mode="VIEW_LAYER"`, `use_sort_alpha=True` 既定にし、ユーザーの既存 workspace 内 Outliner には触らない。
   - **(b)** N パネルの「Outliner を B-MANGA 表示に切替」ボタンで明示的に既存 area の設定を変更する。直前設定を `area["bmanga_outliner_backup"]` に退避し、復元ボタンも提供する。
...
4. 既存 `bmanga_layer_stack` は残しつつ、D&D は非推奨表示にする。
...
目的: GP 内部レイヤー管理から、1 GP Object = 1 B-MANGA レイヤーへ移行する。

作業:

1. 新規 GP レイヤー作成時に新 GP Object を作る。
2. 既存 `bmanga_master_sketch` 内 GP レイヤーを GP Object 群へ移行する operator を作る。
...
2. 既存 raster plane を B-MANGA outliner model に正式登録する。
...
- テキストは Blender Text Object だけでは縦書き、ルビ、縦中横、独自組版の再現が難しいため、当面は B-MANGA typography から生成した画像 plane Object とする。
...
1. 効果線データブロックの kind を 1 つに統一する (推奨: GP Object とし、`bmanga_kind = "effect"` を立てる)。Mesh 効果線は段階的に GP Object 化する operator を提供する。
2. 既存ファイル読込時、GP/Mesh 混在を検出して `bmanga_kind = "effect"` の正規化済み Object として登録する。
3. Outliner 上で `L00NN__effect__放射線` のように prefix 化し、コマ Collection 直下に配置する。
4. 効果線の参照対象 (フキダシ/コマ) との連動 (`effect_line_link_op.py`) を Object Custom Property で表現する (`bmanga_effect_target = "<balloon_id>" | "<coma_id>"`)。
...
1. B-MANGA パネルに作成/削除/前面/背面/詳細/修復ボタンを配置。
2. 現在の `BMANGA_UL_layer_stack` を非表示または読み取り専用へ変更。
...
- B-MANGA パネルはレイヤー作成・詳細編集・順序操作に集中する。
...
| `core/page.py` / `core/coma.py` | Collection と安定 ID の同期。`BMangaPageEntry.id` (`pNNNN`/`pNNNN-NNNN`) と `BMangaComaEntry.id` (`cNN`) を真の安定 ID として固定。`coma_id` (ファイル stem) は派生のまま |
| `core/image_layer.py` | image plane Object との対応。GPU overlay (`ui/overlay_image.py`) → material node tree への移植が伴う |
| `core/raster_layer.py` | raster plane を正規レイヤー Object 化。既存 `RASTER_Z_LIFT_M = 0.0005` 固定値を `z_index * BMANGA_Z_STEP_M` 由来に置換するマイグレーション |
...
| `utils/object_naming.py` (新設) | Object 名 prefix 生成 / UTF-8 安全切詰め / `bmanga_id` 突合 |
...
| `panels/gpencil_panel.py` | レイヤーリスト (`BMANGA_UL_layer_stack`) は当該ファイルにあるが、Phase 6 で `panels/layer_stack_panel.py` への分離も同時に検討 |
...
- B-MANGA パネルの前面/背面ボタンで Outliner 名 prefix が変わる。
...
| Object が複数 Collection に所属 | 親が曖昧になる | `bmanga_managed=True` の Object のみ自動正規化。`bmanga_no_normalize=True` で退避可能 |
| Object 名 UTF-8 切詰め | Blender 5.1.1 の Object/Collection 名は 255 バイトで黙って切詰め。日本語タイトルは prefix 込で 80 字相当が上限 | `bmanga_title` を custom property に保持し Object 名は派生のみ。生成時に UTF-8 安全切詰め (`utils/object_naming.py`) を通す |
| 同名衝突 (.001 自動付加) | Blender が自動的に `.001` を付ける | 検出 scan は Object 名でなく `bmanga_id` で突合。prefix は B-MANGA 側から `obj.name = canonical` で随時上書き |
...
| depsgraph 再帰 | コールバック内変更で再 fire | guard フラグ + 差分キャッシュ + B-MANGA operator 全体 suppress |
...
| テキスト Object 化 | Blender Text では B-MANGA 組版を再現しづらい | typography 生成画像 plane を採用 |
| コママスク | 任意形状クリップが難しい | ページマスクから段階導入、コマは種別ごとに対応 |
| Outliner 設定上書き | ユーザーの既存 Outliner 設定を勝手に変えると混乱 | 専用 workspace `B-MANGA` を提供 (Phase 0)。既存 area 上書きは Phase 6 で復元機能付きで導入 |
