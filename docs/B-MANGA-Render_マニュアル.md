...
B-MANGA Render は、B-MANGA の出力系を分離した別アドオンです。

主な役割は、コマ用blendファイルのキャラ、背景、効果、魚眼、Pencil+4 などの出力工程を、コマンド型のアクションリストとして組み立て、上から順番に実行することです。

B-MANGA 本体は、ページ一覧ファイルでの作画と、コマ用blendファイルでの 3D ビューポート上モデル配置を担当します。レンダー出力、魚眼レンダリング、複数パス出力は B-MANGA Render を使います。

## インストールと有効化

B-MANGA Render は B-MANGA 本体とは別アドオンです。

### 通常インストール

1. Blender を開きます。
2. プリファレンスを開きます。
3. B-MANGA Render をインストールして有効化します。
4. 3D ビューのサイドバーに「B-MANGA Render」タブが出ていることを確認します。

### 開発中リポジトリから使う場合

B-MANGA リポジトリ内の `addons/b_manga_render` が B-MANGA Render のアドオン本体です。開発中は、Blender の extensions フォルダ内に B-MANGA Render 用のジャンクションを作り、このフォルダを直接参照させます。

B-MANGA 本体のリポジトリ直読み込みとは別に、B-MANGA Render も個別に登録してください。

## どこから使うか

B-MANGA Render は以下から使えます。

| 場所 | 表示 |
| --- | --- |
| 3D ビュー | サイドバーの「B-MANGA Render」タブ |
| ノードエディター | サイドバーの「B-MANGA Render」タブ |
...
![B-MANGA Render の初期プリセット実行例](images/manual/bmanga_render_presets.png)

## 画面構成

### 魚眼出力

上部の「魚眼出力」では、コマ用blendファイル側で設定された魚眼状態と、B-MANGA Render 側でも変更できる縮小設定を表示します。

| 項目 | 説明 |
| --- | --- |
| 魚眼モード | B-MANGA のコマ編集側で設定した魚眼状態を表示します。 |
| 縮小モード | 確認用に低解像度出力します。 |
| 縮小率 | 縮小モード時の出力倍率を指定します。 |
| 魚眼FOV | B-MANGA のコマ編集側で設定した魚眼カメラの画角を表示します。 |
...
初期プリセットには、B-MANGA のコマ用blendファイルで使う出力プリセットと、旧出力シーン互換プリセットが含まれています。ページ一覧全体の出力は B-MANGA 本体のページ出力を使います。
...
2. B-MANGA Render タブを開きます。
3. プリセットを選びます。
4. 「プリセットを実行」を押します。

### 魚眼レンダー

1. B-MANGA のコマ編集側で「魚眼モード」と「魚眼FOV」を設定します。
2. 必要に応じて「ページ画像のスケール」を B-MANGA のコマ編集側で設定します。
3. 確認用に軽く出す場合は B-MANGA Render の「縮小モード」をオンにし、「縮小率」を設定します。
...
B-MANGA のページ一覧ファイルで作成したページ画像や、コマ用blendファイルの出力解像度は、B-MANGA Render の出力前提になります。

コマ用blendファイル側では、ページ画像、下絵、カメラ、出力解像度を確認してください。B-MANGA Render は、それらの状態を前提に、レンダー工程をコマンド順に実行します。

B-MANGA 管理下のコマ用blendファイルでは、出力先の既定はそのコマの `passes` フォルダになります。B-MANGA 本体が作るページ一覧用プレビューとは別の名前で出力します。
...
B-MANGA のコマ編集側で「魚眼モード」と「魚眼FOV」を変更してください。B-MANGA Render では、出力に使う値を読み取り表示します。
