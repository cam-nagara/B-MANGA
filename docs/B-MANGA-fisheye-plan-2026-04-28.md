# B-MANGA 魚眼レンダリング機能 実装計画

作成日: 2026-04-28
対象範囲: コマファイル（`<Work>.bmanga/p<NNNN>/c<NN>/c<NN>.blend`）のみ
方針: A案（eeVR を参考に独自実装、コードは流用しない）

**前提**: [B-MANGA-restructure-plan-2026-04-28.md](B-MANGA-restructure-plan-2026-04-28.md) の構造改修完了後に着手。本計画書は新構造（`p0001/c01/passes/`）を前提に記述。

## 1. 背景と目的

B-MANGA は「ネーム作画用」と銘打っているが、実際の運用では:
- キャラ作画は CLIP STUDIO PAINT で行う
- **背景・小物・効果線・フキダシ・配置済み 3D は B-MANGA 側で本番原稿として Cycles 魚眼レンダリングする**

このため魚眼レンズの「正確さ」が必須で、ネーム用簡易表示では足りない。

現状 B-MANGA には魚眼用 PropertyGroup・apply 関数の下地があるが、Cycles `FISHEYE_EQUISOLID` を使うのみ。
ユーザーの既存ワークフローには以下が含まれており、これらを B-MANGA 側に独自実装する:
...
参考実装の eeVR は GPL v3。**本実装ではコードを取り込まず、公知の数式（r = 2f·sin(θ/2) ほか）と公知の手法（cube map 6 面 + 再投影）を独自に再実装する**。これによりライセンス汚染を回避し、B-MANGA 既存の SPDX:GPL-2.0-or-later を維持する。

## 2. 投影方式の確定（重要）

### 採用する投影は 2 種：Equidistant と Equisolid（Cycles 自動連動）

ユーザーは作品／コマごとに Cycles の `panorama_type` を **`FISHEYE_EQUIDISTANT`（魚眼・等距離射影）** と **`FISHEYE_EQUISOLID`（魚眼・等立体角射影）** で使い分ける運用。B-MANGA の EEVEE 魚眼プレビューはこれを **手動切替させずカメラ設定に自動追従** させる。
...
- B-MANGA の EEVEE プレビューは対応せず、警告ログ + Equisolid フォールバック表示
- UI 側に「現在のカメラパノラマタイプは EEVEE プレビュー未対応です。Cycles で本番レンダしてください」を出す
- Cycles 本番レンダ自体は B-MANGA が触らないため正常動作する
...
5. **計画書出力先**: 本ファイル（`docs/B-MANGA-fisheye-plan-2026-04-28.md`）
6. **構造改修依存**: 本計画は [B-MANGA-restructure-plan-2026-04-28.md](B-MANGA-restructure-plan-2026-04-28.md) の完了を前提とする。構造改修前に着手しない
...
- `operators/fisheye_op.py` — オペレータ
  - `BMANGA_OT_fisheye_render_image` — 1 枚レンダ（cube → 再投影 → 保存）
  - `BMANGA_OT_fisheye_render_faces` — 6 面のみレンダ（再投影なし）
  - `BMANGA_OT_fisheye_assemble` — 既存の 6 面画像から再投影のみ実行
  - `BMANGA_OT_fisheye_save_pencil4_widths` — Pencil+4 線幅をスナップショット
- `panels/fisheye_panel.py` — N パネル UI（既存 panel_camera_panel に統合する案も）

既存ファイル拡張（**ファイル名は構造改修後の新名**を記載）:
- `core/coma_camera.py`（旧 `core/panel_camera.py`） — 既存の `BMangaComaCameraSettings`（旧 `BMangaPanelCameraSettings`）に魚眼レンダ用プロパティ追加
  ...
  - `fisheye_output_dir: StringProperty` — 出力先（既定 `//passes/`、コマファイルは `c01.blend` なので `//` は `c01/` を指し、最終的に `<Work>.bmanga/p<NNNN>/c<NN>/passes/` になる）
  - `fisheye_cube_keep: BoolProperty` — cube map 6 面中間画像を残すか（初期 True、`passes/cube/` に保存）
  - **投影方式は持たない**: `cam.data.panorama_type` を真とするため B-MANGA 側にプロパティを置かない（二重管理防止）
...
中間画像の保存先: `<Work>.bmanga/p<NNNN>/c<NN>/passes/cube/` に
...
### 4.4 魚眼再投影シェーダ（Equidistant / Equisolid 両対応、B-MANGA オリジナル実装）

eeVR の `commdef`/`dome`/`fetch_*` は同梱しない。以下のように **B-MANGA 専用に書き直す**。
...
- コレクション名 `コマ枠` は **B-MANGA 既存規約**として存在することを前提にする
...
- **視野角 (°)**: B-MANGA UI ラベルは度（°）固定。内部値はラジアン（Blender カメラ標準）で保持し、UI のみ度表記に変換する。
...
**前提**: [B-MANGA-restructure-plan-2026-04-28.md](B-MANGA-restructure-plan-2026-04-28.md) の Phase R1〜R5 が完了していること。本魚眼 Phase は構造改修後の `coma_camera.py` 等を対象にする。
...
- `BMANGA_OT_fisheye_render_image` 実装（実行時に panorama_type を読んで自動分岐）
- 中間ファイル経路（temp dir → 結果 PNG）
- 投影方式の単体テストを **両投影とも** 通す

### Phase F5 — 6 面分割／組立オペレータ
範囲:
- `BMANGA_OT_fisheye_render_faces`（6 面のみ書き出し）
- `BMANGA_OT_fisheye_assemble`（既存 6 面から再投影）
...
- 実装ファイルのモジュール docstring に **以下のような 1 行表記を入れる**（法的義務はないが、第三者監査時の証跡として推奨）:
  ```python
  """B-MANGA fisheye reprojection (Equidistant / Equisolid).
...
| 既存自作 .py との設定衝突 | 二重制御 | B-MANGA 内では完結。自作 .py は同梱せず、ユーザーが必要なら別途同居 |
...
### 9.2 実機テスト（要手動立会い）
- Blender 5.1 起動 → 任意の `c<NN>.blend` を開く → 魚眼モード ON → Cycles レンダ → 出力 PNG が `<Work>.bmanga/p<NNNN>/c<NN>/passes/` に期待通り出力
