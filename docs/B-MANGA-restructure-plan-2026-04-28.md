# B-MANGA ディレクトリ構造改修 計画

作成日: 2026-04-28
位置づけ: 魚眼レンダリング実装（[B-MANGA-fisheye-plan-2026-04-28.md](B-MANGA-fisheye-plan-2026-04-28.md)）の前提改修。本改修完了後に魚眼実装に着手する。
...
### 3.3 副産物
- コマ単位の魚眼出力: `<Work>.bmanga/p0001/c01/passes/`
- cube map 中間画像: `<Work>.bmanga/p0001/c01/passes/cube/`
...
| ファイル | 改修内容 |
|---|---|
| [utils/paths.py](D:/Develop/Blender/B-MANGA/utils/paths.py) | `PAGES_DIR_NAME` / `PANELS_DIR_NAME` 削除、`page_dir` / `panels_dir` を `coma_dir` 中心に再設計、`format_panel_stem` を `format_coma_id` に改名し 2 桁化、正規表現更新 |
| [io/work_io.py](D:/Develop/Blender/B-MANGA/io/work_io.py) | パス生成箇所更新 |
| [io/page_io.py](D:/Develop/Blender/B-MANGA/io/page_io.py) | 同上 |
| [io/panel_io.py](D:/Develop/Blender/B-MANGA/io/panel_io.py) | ファイル名 `panel_io.py` → `coma_io.py` に改名検討、内部もコマ命名に |
| [io/meldex_receiver.py](D:/Develop/Blender/B-MANGA/io/meldex_receiver.py) | パス処理更新 |
| [operators/work_op.py](D:/Develop/Blender/B-MANGA/operators/work_op.py) | 新規作成・保存・読込処理 |
| [operators/page_op.py](D:/Develop/Blender/B-MANGA/operators/page_op.py) | ページ追加削除 |
| [operators/spread_op.py](D:/Develop/Blender/B-MANGA/operators/spread_op.py) | 見開き処理 |
| [operators/layer_stack_op.py](D:/Develop/Blender/B-MANGA/operators/layer_stack_op.py) | 中身確認の上で更新 |
| [utils/handlers.py](D:/Develop/Blender/B-MANGA/utils/handlers.py) | load_post の path resolve |
| [utils/page_range.py](D:/Develop/Blender/B-MANGA/utils/page_range.py) | ページ範囲処理 |
| [utils/panel_camera_refs.py](D:/Develop/Blender/B-MANGA/utils/panel_camera_refs.py) | 下絵参照のパス |
...
ドキュメント:
- [docs/B-MANGA-overview-plan.md](D:/Develop/Blender/B-MANGA/docs/B-MANGA-overview-plan.md) — アーキ図と本文を新構造に更新
- [docs/B-MANGA-plan.md](D:/Develop/Blender/B-MANGA/docs/B-MANGA-plan.md) — 同様に更新
- [docs/B-MANGA_設計意図.md](D:/Develop/Blender/B-MANGA/docs/B-MANGA_設計意図.md) — 命名規約を新規追記

既存データ:
- 旧構造の `作品テスト.bmanga` 等は破棄（ユーザー承諾済）
- マイグレーションスクリプトは作らない
...
- Scene プロパティ名 `bmanga_panel_camera_*` 等も `bmanga_coma_camera_*` に
- core 層も同時改名

### Phase R4 — UI 層書換え
- N パネル UI のラベル「パネル」を「コマ」に統一
- bl_idname の prefix 更新（`BMANGA_OT_panel_*` → `BMANGA_OT_coma_*`）
- keymap も追従
...
| Blender 側 `bpy.types.Panel`（UI 部品）との混同が残る | 命名衝突 | UI クラスは Blender 慣習通り `BMANGA_PT_*` で命名（既存通り）、コマ概念は `BMangaComa*` プレフィックスに統一。両者を別名前空間に分離 |
...
- 新規作品作成で `<Work>.bmanga/work.blend` のみ生成、`pages/` ディレクトリは作られない
- ページ追加で `<Work>.bmanga/p0001/page.json` が作成される
- コマ追加で `<Work>.bmanga/p0001/c01/c01.blend` + `c01.json` が作成される
- コマ追加 99 個まで成功、100 個目で明確なエラー
- 見開きページ追加で `<Work>.bmanga/p0020-0021/` が作成される
...
本改修完了後、[B-MANGA-fisheye-plan-2026-04-28.md](B-MANGA-fisheye-plan-2026-04-28.md) の Phase F1+F2 に進む。魚眼計画書側も新パス記法（`c01/passes/`）と新ファイル名（`coma_camera.py` 等）に追従済み。

## 10. 着手前の追加確認事項

- 本計画書には記述していないが Phase R2 着手前に確認したいこと:
  - Meldex 連携が現在使われているか（[io/meldex_receiver.py](D:/Develop/Blender/B-MANGA/io/meldex_receiver.py) の呼出元と使用状況）
