# B-MANGA ラスター描画レイヤー導入計画 (2026-04-28)
...
| 配置画像 / image | 既存の `BMangaImageLayer`。スキャンラフや写真の配置用。 |
...
### 2.1 PropertyGroup `BMangaRasterLayer`
...
`Scene.bmanga_raster_layers : CollectionProperty(type=BMangaRasterLayer)` として作品全体の単一コレクションで持つ。所属 (`scope` / `parent_*`) はこのコレクション内のメタで判定。
...
- `utils/layer_stack.py` の `resolve_stack_item` に kind=`raster` の解決枝を追加。`item.key` を UUID とし、`bmanga_raster_layers` から該当エントリを引いて返す。
...
現状の `BMANGA_MT_layer_stack_add` は単一 Menu で `_ADD_KIND_ITEMS` をフラットに列挙する構造。raster は DPI / bit 深度のバリエーションがあるため **入れ子サブメニューを新設**する。

- `_ADD_KIND_ITEMS` に `"raster"` を追加するが、raster だけは `layout.operator` で直接出さず、`layout.menu("BMANGA_MT_layer_stack_add_raster")` でサブメニューを開く分岐を `BMANGA_MT_layer_stack_add.draw` 内に追加する。
- `_ADD_KIND_ICONS` に `"raster": "BRUSH_DATA"` を追加。
- 新規 Menu クラス `BMANGA_MT_layer_stack_add_raster` (operators/layer_stack_op.py に同居) を追加し、以下のプリセットを並べる:
  - 300dpi / グレー 8bit (既定)
  - 150dpi / グレー 8bit
  - 300dpi / 1bit
  - 150dpi / 1bit
  - カスタム... (DPI / bit 深度をダイアログで指定)
- 各プリセットは `bmanga.raster_layer_add` を呼び、`dpi` / `bit_depth` プロパティを設定。
...
- visible / locked
- opacity (slider)
- dpi (**読み取り専用ラベル + 「リサンプル...」ボタン**。`layout.prop` 直書きはしない。値変更はオペレータ `bmanga.raster_layer_resample` 経由でのみ行い、ピクセルデータの Lanczos 再サンプリングと同期させる)
- bit_depth (gray8 / gray1。変更時は `bmanga.raster_layer_set_bit_depth` 経由)
- line_color
...
- `bmanga.raster_layer_add` オペレータを直接呼ぶショートカットも準備 (デフォルトキーマップは未割り当て、ユーザーが設定可能)。
...
| bl_idname | 機能 |
| -- | -- |
| `bmanga.raster_layer_add` | UUID 採番 / Image 生成 / PNG 即時書き出し / Plane 生成 / レイヤースタック挿入 |
| `bmanga.raster_layer_remove` | アクティブラスターを削除 (Image・Plane・PNG・メタの一斉削除、Undo 対応) |
| `bmanga.raster_layer_select` | アクティブ化。kind=`raster` への切替 |
| `bmanga.raster_layer_paint_enter` | 該当 Plane を active 化、Texture Paint モードへ |
| `bmanga.raster_layer_paint_exit` | OBJECT モードへ戻す。AGENTS.md「ツール継続」原則に従い、別ツールへ切替時のみ呼ぶ |
| `bmanga.raster_layer_resample` | DPI 変更時に既存画素を Lanczos でリサンプル |
| `bmanga.raster_layer_set_bit_depth` | gray1 / gray8 切替 (gray1 化時はしきい値ダイアログ) |
| `bmanga.raster_layer_save_png` | 編集中の Image を `<work>/raster/<uuid>.png` に書き出し (gray8 + α の PNG。Ctrl+S フックから呼ぶ) |
| `bmanga.raster_layer_move_scope` | scope を page ↔ master 付け替え |
| `bmanga.raster_layer_move_parent` | parent_kind/parent_key を変更 (page → panel、別ページへ移動など) |
...
- B-MANGA 用フラグ: `obj["bmanga_raster_id"] = <uuid12>`、master scope の場合は `obj["bmanga_raster_master_page"] = <page_id>` も付与して識別。
...
- 描画中のグレースケール強制:
  ...
  - `bmanga.raster_layer_paint_enter` 内で `tool_settings.image_paint.brush.color` 監視ハンドラを設定し、変更されたら R=G=B の平均値で上書き。退出時にハンドラ解除。
...
- 別ツールに切り替わる時、または `bmanga.gpencil_master_mode_set` 系が呼ばれた時のみ OBJECT モードへ戻す。
...
- 既存 [`operators/layer_move_op.py`](D:\Develop\Blender\B-MANGA\operators\layer_move_op.py) を拡張。
- 操作:
  - `bmanga.raster_layer_move_scope` : scope を page ↔ master に切替。
  - `bmanga.raster_layer_move_parent` : parent_kind / parent_key を切替 (page → panel、別ページ、別コマ)。
...
- アンロード手順 (データ消失防止のため必ず以下の順):
  1. 対象 Image の `is_dirty` を確認。dirty なら `bmanga.raster_layer_save_png` 相当のロジックで PNG に書き出す。
...
- ページ切替フックは `bmanga_active_page_index` の update コールバック、または `utils/handlers.py` の既存 page change ハンドラに同居。
...
- 既存の `bmanga.gpencil_master_mode_set` (panels/gpencil_panel.py) は GP 専用の wrapper オペレータ。raster 用に **同等の wrapper `bmanga.raster_layer_mode_set`** を新設する。
...
- 既存ツール切替ボタン (アプリバー等) からモードを切り替える時、現在のレイヤーが raster なら raster wrapper、gp なら gp wrapper を呼ぶ分岐を `BMANGA_OT_gpencil_master_mode_set.execute` 周辺に追加 (または新たに `bmanga.layer_aware_mode_set` 統合 wrapper を作る)。
...
新規ディレクトリ `<work>/raster/` を作る。`pages/` 配下や `panels/` 配下には何も追加しない (集約方式)。`utils/paths.py` に `RASTER_DIR_NAME = "raster"` と `raster_dir(work_dir)` / `raster_png_path(work_dir, uuid)` ヘルパを追加し、ディレクトリ生成は `create_bmanga_skeleton` で他のサブディレクトリ (assets/scenario/exports) と並列で行う。
...
| R3 | Plane の Z 順 | Blender ビューポート上でレイヤースタックの前後と Z 順を一致させる必要 | `utils/handlers.py` に `_bmanga_on_layer_stack_reorder` ハンドラを追加、`bmanga.layer_stack_move` 等のオペレータ完了時に呼ぶ |
...
- scope / parent 移動ツール (`bmanga.raster_layer_move_scope` / `bmanga.raster_layer_move_parent`)
...
| 区分 | パス | 操作 |
| -- | -- | -- |
| 新規 | `core/raster_layer.py` | PropertyGroup `BMangaRasterLayer` |
...
| 編集 | `operators/layer_stack_op.py` | `_ADD_KIND_ITEMS` に raster 追加、`_ADD_KIND_ICONS` 追加、`BMANGA_MT_layer_stack_add.draw` で raster は `layout.menu()` 分岐、新規 Menu `BMANGA_MT_layer_stack_add_raster`、`_add_by_kind` に raster 分岐 |
...
| 編集 | `utils/handlers.py` | `_bmanga_on_save_pre` 内で raster の dirty PNG 書き出し追加、page change ハンドラに master scope Plane 同期と遅延ロード追加、layer stack reorder ハンドラ |
| 編集 | `utils/paths.py` | `RASTER_DIR_NAME` / `raster_dir` / `raster_png_path` / `raster_trash_dir` 追加 |
| 編集 | `io/work_io.py` (`create_bmanga_skeleton`) | `raster/` ディレクトリ作成を追加 |
| 編集 | `utils/color_space.py` | `linear_to_srgb` が未実装なら追加 (既存確認の上) |
| 編集 | `panels/gpencil_panel.py` の `BMANGA_OT_gpencil_master_mode_set` | 現在のレイヤーが raster なら raster mode_set wrapper を呼ぶ分岐を追加 (または上位で振り分ける別オペレータを新設) |
