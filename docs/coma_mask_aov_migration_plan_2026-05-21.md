...
旧 `c00.blend` テンプレートおよび B-Name-Render プリセットは、 ベイク済みの固定 PNG (`コマ枠.png`) を「コマの形」と「用紙サイズ」の正本として扱っていた。 現行 B-Name はコマ形状をページ一覧側の実オブジェクトで管理しているため、 静止 PNG が陳腐化し、 形状変更や用紙サイズ変更に追従しない。

本計画は、 静止 PNG を撤去し、 B-Name が生成するメッシュ + AOV パイプラインへ移行する。
...
1. **メッシュ・マテリアル・コレクション・view layer・AOV の命名は既存を踏襲する**:
   ...
   - view layer: `コマ枠`
   - AOV (COLOR): `コマ枠拡張`
2. **メッシュ頂点は B-Name が生成する**: 現在コマの多角形 (mm) を z=0 平面に配置。 配置は overview と同じ世界座標 (ページ位置にぴったり)。
...
- `utils/handlers.py::_bname_on_load_post`: コマ用 blend 判定の分岐に `ensure_coma_mask_mesh` 呼び出しを追加。
- `utils/handlers.py::_bname_on_save_pre`: 同じく save_pre のコマ判定分岐に追加。
...
### Phase 3: B-Name-Render プリセット

`addons/b_name_render/preset_library.py`:
- `_page_output` と `_all_output` (旧 `ページ` / `全コマ統合` 経路) は AGENTS.md / 監査ドキュメントに従い、 現行 B-Name 仕様外として既存どおり `旧出力シーン互換: ...` 表記で残す (変更最小)。
...
- コマ用 blend テンプレートを開く。
- B-Name の `ensure_coma_mask_mesh` を呼ぶ。
...
`c00.blend.bak_YYYYMMDD` を `c00.blend` にコピーして戻す。 B-Name コード側は `mark_all_externally_finished` 同様の安全機構が無いため、 該当コミットを `git revert` で戻す。
