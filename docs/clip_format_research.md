...
B-MANGA から CLIP STUDIO PAINT が読み込める `.clip` ファイルを書き出す。最低目標は次の2点:

- **B-MANGA のコマを CSP の「コマフォルダ」として出力**（コマ枠線つき・マスクつきの折りたたみ可能フォルダ）
- **B-MANGA の Grease Pencil（GP）を CSP の「ベクターレイヤー」として出力**
...
2. **P2+P3**：B-MANGA のコマを **コマフォルダの「枠＋空ラスタ中身」**として並べる。コマ枠線は最初は「黒1px線をラスタライズしただけのダミー」でもCSPで開く（Offscreenタイル形式が成功した時点でゴール）。
...
| 3D/特殊データ | サンプルにはデッサン人形(`CLIP_STUDIO_3D_DATA2`)が含まれる | B-MANGA 出力では 3D 関連テーブルは空のまま参照しない |
...
| 案 | 内容 | 推奨度 |
|---|---|---|
| **A** | PSD のコマフォルダ階層化を先に実装（B-MANGA のコマ→PSDレイヤーフォルダ） | ★★★ 最短で実用効果大 |
