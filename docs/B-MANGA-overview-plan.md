...
- `Scene.bmanga_work` (PointerProperty) — work.blend 内で永続。`work.pages[i].comas[j]` に全ページ・全コマのメタを保持
- ページ/コマメタは常に JSON が真で、`load_post` で Scene に再構築 (既存 `_reload_all_pages_panels` を流用)
- `Scene.bmanga_overview_mode` — overview 表示モード。本計画後は既定 True、ユーザーは単ページ拡大表示へトグル可能
...
- `operators/work_op.py`:
  - 旧案 `work_new`: work.blend を `<work>.bmanga/work.blend` に保存。page.blend の保存は行わない。最初のページ 0001 と基本枠コマは従来通り作成 (Scene に乗る)
...
- `ui/overlay.py`:
  - Scene.bmanga_overview_mode の default を True にする
  - コマ編集モード中は overview 描画をスキップし従来通りアクティブコマの 2D 表示のみ
- `operators/view_op.py`:
  - `bmanga.view_fit_all` / `bmanga.view_overview_toggle` は維持。`bmanga.view_fit_page` は単一ページ表示トグル用途に残す
...
- **コマ一覧 UIList にダブルクリック → `bmanga.enter_coma_mode`** をハンドラで結線:
  - UIList.draw_item 内で行クリック判定は難しいため、代替として以下どちらかで実装:
    - UIList 行に小さな「コマ編集へ」アイコンボタンを追加 (確実)
    - または N パネル側に「選択中コマを編集」ボタン (既存の「コマ編集へ」を流用)
  - ビューポート上のコマダブルクリックは keymap で `LEFTMOUSE` `DOUBLE_CLICK` を `bmanga.enter_coma_mode` に結線、invoke 時に world 座標→対象コマ逆引き
- ショートカット keymap に `LEFTMOUSE` `DOUBLE_CLICK` を `bmanga.enter_coma_mode` 割当 (「B-MANGA Viewport」keymap、既存 watcher 制御に従う)

### 範囲外 (Phase 2 以降)
- 各ページの GP オブジェクト自動生成と Collection 配置
- overview 上での GP 描画
- 縦書きテキスト / フキダシ / 画像レイヤーの overview 対応
- per-page .blend で作成された旧作品の自動マイグレーション (既存 `作品テスト.bmanga` は破棄)
...
- [ ] `作品を新規作成` で `.bmanga/work.blend` が作成される (page.blend は作成されない)
...
- `BMANGA_OT_gpencil_setup` の位置づけを再検討 (ページ単位に変える / 廃止)
- GP パネル (`panels/gpencil_panel.py`) を複数ページ対応にリファイン
- `load_post` ハンドラで GP オブジェクト ↔ ページ対応の整合確認

### 新 operator
- `bmanga.gpencil_page_ensure`: アクティブページに GP がなければ生成
- `bmanga.gpencil_follow_cursor`: 自動切替 ON/OFF トグル (preferences に連動可)
...
| ダブルクリック keymap が他アドオンと衝突 | 誤作動 | 既存の `watcher` による B-MANGA タブ active 時のみ有効化に乗せる |
| 旧作品の破棄リスク | データ喪失 | ユーザー承諾済 (作品テスト.bmanga 破棄 OK の明示あり) |
