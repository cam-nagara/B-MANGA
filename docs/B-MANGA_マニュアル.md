...
最終更新: 2026-07-11
対象: B-MANGA 0.6.480 / Blender 5.1.2
...
各本文にはフキダシとテキストが作られます。シナリオの「タイプ」と完全に同じ名前のフキダシプリセットまたはテキストプリセットがある場合、それぞれの設定が使われます。フキダシプリセットがない場合は標準フキダシになります。改行とルビはMeldexの表示を保ち、フキダシは本文とルビが収まる大きさへ調整されます。
