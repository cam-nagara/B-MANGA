...
最終更新: 2026-07-14
対象: B-MANGA 0.6.503 / Blender 5.1.2
...
| Meldex 連携 | Meldexから受信する場合に有効化し、ポートと接続トークンを確認します。受信は既定でオフ、対応する連携データのルビ表示設定は既定で適用します。 |
...
「Meldexのルビ表示設定を適用」がオンの場合、対応するMeldexから送られた書字方向、サイズ、親文字との間隔、字間、ルビ行間、配置方法、小書き仮名、ルビ用フォントを取り込みます。オフの場合はB-MANGAのテキストプリセットを使います。旧形式の連携データは従来どおり本文、ルビ範囲、読みを取り込み、見た目はB-MANGA側で決まります。

シナリオのページ数がB-MANGAの既存ページ数より多い場合だけ、不足ページが末尾へ追加されます。コマは自動生成されません。

各本文にはテキストが作られ、シナリオの「タイプ」が話者名になります。「タイプ」と完全に同じ名前のテキストプリセットがある場合はその設定を使い、そのプリセットの「リンクフキダシプリセット」が空ならフキダシを作りません。リンク先が指定されている場合、または一致するテキストプリセットがない場合は、対応するフキダシプリセットを使ってフキダシも作ります。フキダシプリセットがない場合は標準フキダシになります。改行とルビはMeldexの表示を保ち、フキダシは本文とルビが収まる大きさへ調整されます。

同じシナリオを再送すると、前回取り込んだ同じ本文が更新されます。手作業で作ったページ、コマ、フキダシ、テキストは削除されません。
...
| オーバーレイ表示 | B-MANGA の選択枠やハンドルなどの補助表示を切り替えます。 |
...
画面上部には対象名、表示、ロックを一度だけ表示し、その直下にプリセットをサイドバーと同じ一覧形式で表示します。一覧右側のアイコンから、追加、削除、上下移動、現在設定で上書き、名前変更、複製を行えます。詳細設定内でプリセット行を選ぶと、開いた時に確定したレイヤーへ直ちに適用されます。別のレイヤーを途中で選択しても編集対象は変わりません。リンク中のレイヤーは下部へまとめます。

プリセットを選択または保存したあとで、そのプリセットに保存される設定を変更してから別のプリセットへ切り替えると、現在の設定を保存せずに切り替えてよいか確認します。「キャンセル」を選ぶと、設定とプリセット一覧の選択行は切り替え前のまま残ります。変更を元の値へ戻した場合や、現在の設定をプリセットへ追加・上書き保存した場合は確認を表示しません。
...
ルビ設定は「サイズ（親文字比%）」「親文字との間隔」「ルビの字間」「ルビ行の行間」「配置方法」「小書き仮名」「ルビ用フォント」の順です。間隔と字間は親文字に対する相対値なので、文字サイズを変えても比率を保ちます。ルビの追加・編集ではグループ、モノ、熟語を選べます。Meldex連携v2から内訳が送られたモノ／熟語ルビは、親文字ごとの対応を使って組版します。
