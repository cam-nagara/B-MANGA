...
最終更新: 2026-08-08
対象: B-MANGA Next 0.6.607 / Blender 5.2 LTS（UIに変更はなく、画面例は Blender 5.1.1 実機のまま流用）
...
ツールボタンの下には、使用中ツールのプリセット一覧が表示されます。フキダシ、テキスト、効果線、囲い塗り、グラデーション、パターンカーブ、枠線に加えて、グリースペンシル描画中はグリースペンシルツールプリセットが表示されます。

![B-MANGA パネルのツールとビュー](images/manual/bmanga_panel_tools_view.png)

## グリースペンシルツールプリセット

グリースペンシル描画・スカルプト中は、ツールボタンの下にグリースペンシルツールプリセットの一覧が表示されます。他のツールのプリセットが「レイヤーの設定」を保存するのに対し、このプリセットは「ツールの設定」を保存します。一覧でプリセットを選ぶと、必要なモードとツール・ブラシへ切り替わり、保存されている設定がすぐに適用されます。

1つのプリセットには、グリースペンシルの機能1つとその詳細設定を保存します。機能は次の5種類から選べます。

| 機能 | 内容 |
| --- | --- |
| ブラシ | ドローモードのブラシで線を描きます。使用ブラシ、サイズの基準、サイズ、強さ、筆圧、ストロークタイプ、キャップ、硬さ、手ブレ補正を保存します。 |
| フィル | 囲まれた領域を塗りつぶします。方向、計算方式、精度、拡張、サイズの基準、線の太さ、すき間閉じを保存します。 |
| トリム | ドラッグした線でストロークを切り取ります。アクティブレイヤーのみ、キャップを保持を保存します。 |
| 消しゴム | ストロークを消します。消しゴムモード（ポイント / ディゾルブ / ストローク）、サイズ、強さ、筆圧、アクティブレイヤーのみ、キャップを保持を保存します。 |
| グラブ | スカルプトモードのグラブブラシで、描いたストロークをつかんで動かします。サイズ、強さ、筆圧を保存します。 |

ブラシとフィルの「サイズの基準」は、ページ基準（mm）と画面基準（px）を選べます。ページ基準はズームに関係なくページ上で常に同じ太さで描く方式（Blender同梱ブラシの既定）、画面基準は画面上のピクセル数で太さを決める方式です。「ブラシ（標準）」と「フィル（標準）」はページ基準1mmです。消しゴムとグラブは画面基準（px）で保存されます。

最初から「ブラシ（標準）」「フィル（標準）」「トリム（標準）」「消しゴム（標準）」「グラブ（標準）」の5つが用意されているため、一覧はそのままツール切替としても使えます。

一覧右側のボタンで、追加、削除、上下移動、名前変更、複製、上書き保存を行えます。＋ボタンと上書き保存は、いま画面で使っているツール・ブラシの設定（現在のモード・ツール・ブラシの状態）をそのままプリセットにします。プリセット行のアイコンから詳細設定を開くと、機能の選択と、機能ごとの詳細設定を編集できます。
