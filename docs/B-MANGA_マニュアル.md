...
「Meldexのルビ表示設定を適用」がオンの場合、書字方向、サイズ、親文字との間隔、字間、
ルビ行間、配置方法、小書き仮名、ルビ用フォント、ルビ種類を取り込みます。オフの場合は
B-MANGAのテキストプリセットを使います。旧形式の連携データは従来どおり本文、ルビ範囲、
読みを取り込みます。旧Meldexの3.5px互換距離は共通の相対値へ自動変換されるため、
ファイル読込とMeldexからの直接送信でルビ位置は変わりません。
