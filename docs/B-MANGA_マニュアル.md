...
| Meldex 連携 | Meldexから受信する場合に有効化し、ポートと接続トークンを確認します。受信は既定でオフです。取込後のテキストとルビの表示にはB-MANGAのテキストプリセットを使います。 |
...
ファイル読込／直接送信のどちらでも、書字方向、本文・ルビのサイズ、親文字との間隔、字間、
ルビ行間、配置方法、小書き仮名、ルビ用フォント、既定ルビ種類はB-MANGAのテキストプリセットを
使います。Meldex側の表示設定は取り込みません。本文、ルビ範囲、読み、明示されたルビ種類・
由来・優先順位・熟語内訳はシナリオ内容として取り込みます。
