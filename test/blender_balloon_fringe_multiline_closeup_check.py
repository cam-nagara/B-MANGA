...
走らせ方:
  & "C:\\Program Files\\Blender Foundation\\Blender 5.1\\blender.exe" --background --python ^
    "d:/Develop/Blender/B-Name/test/blender_balloon_fringe_multiline_closeup_check.py"
"""
...
_OUT_ENV = os.environ.get("BNAME_FRINGE_CLOSEUP_OUT", "")
_OUT_PATH = Path(_OUT_ENV) if _OUT_ENV else Path(tempfile.mkdtemp(prefix="bname_fringe_closeup_"))


def _load_addon():
    spec = importlib.util.spec_from_file_location(
        "bname_dev_fringe_closeup",
        ROOT / "__init__.py",
        submodule_search_locations=[str(ROOT)],
    )
    mod = importlib.util.module_from_spec(spec)
    sys.modules["bname_dev_fringe_closeup"] = mod
...
def main() -> None:
    temp_root = Path(tempfile.mkdtemp(prefix="bname_fringe_closeup_work_"))
    _OUT_PATH.mkdir(parents=True, exist_ok=True)

    bpy.ops.wm.read_factory_settings(use_empty=True)
    _load_addon()
    result = bpy.ops.bname.work_new(filepath=str(temp_root / "FringeCloseupCheck.bname"))
    assert "FINISHED" in result, result

    from bname_dev_fringe_closeup.core.work import get_work
    from bname_dev_fringe_closeup.operators import balloon_op
    from bname_dev_fringe_closeup.utils import balloon_curve_object
    from bname_dev_fringe_closeup.utils.layer_hierarchy import page_stack_key
