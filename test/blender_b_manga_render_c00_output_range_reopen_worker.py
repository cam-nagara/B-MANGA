...
def main() -> None:
    blend_path = Path(os.environ["BMANGA_RANGE_REOPEN_BLEND"])
    json_path = Path(os.environ["BMANGA_RANGE_REOPEN_JSON"])
    bpy.ops.wm.open_mainfile(filepath=str(blend_path))
    bmanga = _load_package("bmanga_dev_range_reopen", ROOT)
    render = _load_package("bmanga_render_range_reopen", ROOT / "addons" / "b_manga_render")
    try:
        scene = bpy.context.scene
        from bmanga_dev_range_reopen.utils import coma_camera
