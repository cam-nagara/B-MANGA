...
「B-Name パネルを開いている間ずっとビューポートが再描画され続ける」点滅ループに
なっていた (実測 約16回/秒)。
...
def _load_addon():
    spec = importlib.util.spec_from_file_location(
        "bname_dev_layer_loop",
        ROOT / "__init__.py",
        submodule_search_locations=[str(ROOT)],
    )
    mod = importlib.util.module_from_spec(spec)
    sys.modules["bname_dev_layer_loop"] = mod
...
def main() -> None:
    temp_root = Path(tempfile.mkdtemp(prefix="bname_layer_loop_"))
    mod = None
    try:
        mod = _load_addon()
        bpy.context.scene.bname_overview_mode = True
        if "FINISHED" not in bpy.ops.bname.work_new(
            filepath=str(temp_root / "LayerLoop.bname")
        ):
            raise AssertionError("作品作成に失敗しました")

        from bname_dev_layer_loop.utils import layer_stack as LS
        ...
        print("BNAME_LAYER_PANEL_NO_REDRAW_LOOP_OK", flush=True)
    finally:
        if mod is not None:
            mod.unregister()
