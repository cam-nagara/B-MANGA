"""Blender 実機用: B-MANGA Render 分離の登録確認."""
...
def main() -> None:
    ...
    try:
        ...
        assert getattr(bpy.types, "BMANGA_OT_export_pdf", None) is not None
        assert "bmanga_dev.operators.io_op" in sys.modules
        assert "bmanga_dev.panels.export_panel" in sys.modules

        render = _load_package("bmanga_render_dev", ROOT / "addons" / "b_manga_render")
        assert getattr(bpy.types, "BMANGA_RENDER_PT_main", None) is not None
        assert getattr(bpy.types.Scene, "bmanga_render_state", None) is not None
        ...
        from bmanga_render_dev import command_runner, core, eevr_bridge

        scene = bpy.context.scene
        cam_data = bpy.data.cameras.new("BMangaRenderFisheyeCamera")
        cam_data.panorama_type = "FISHEYE_EQUISOLID"
        cam = bpy.data.objects.new("BMangaRenderFisheyeCamera", cam_data)
        ...
        scene.bmanga_coma_camera_fisheye_fov = 2.4
        core._apply_output_resolution_mode(scene)
        assert core.fisheye_enabled(scene), "B-MANGA側の魚眼モードをB-MANGA Renderが認識していません"
        ...
        assert getattr(bpy.types, "BMANGA_OT_coma_camera_reload_backgrounds", None) is None
        ...
        assert abs(float(scene.bmanga_coma_camera_fisheye_fov) - 2.8) < 1.0e-6
        ...
        assert abs(float(scene.bmanga_coma_camera_settings.bg_images_scale) - 1.4) < 1.0e-6
        result = bpy.ops.bmanga_render.set_reduction_scale(percentage=50.0)
        assert result == {"FINISHED"}, result
        assert abs(float(scene.preview_scale_percentage) - 50.0) < 1.0e-6
        assert abs(float(scene.bmanga_coma_camera_preview_scale_percentage) - 50.0) < 1.0e-6
        ...
        result = bpy.ops.bmanga_render.command_add(command_type="RENDER")
        assert result == {"FINISHED"}, result
        ...
        result = bpy.ops.bmanga_render.command_card_click(index=0)
        ...
        result = bpy.ops.bmanga_render.preset_run()
        assert result == {"FINISHED"}, result
