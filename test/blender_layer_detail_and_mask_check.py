...
def _load_addon():
    spec = importlib.util.spec_from_file_location(
        "bmanga_dev_layer_mask",
        ROOT / "__init__.py",
        submodule_search_locations=[str(ROOT)],
    )
    mod = importlib.util.module_from_spec(spec)
    sys.modules["bmanga_dev_layer_mask"] = mod
...
def _stack_index(context, kind: str, key: str) -> int:
    from bmanga_dev_layer_mask.utils import layer_stack as layer_stack_utils
...
def _assert_detail_menu(context, kind: str, key: str, selection_key: str) -> None:
    from bmanga_dev_layer_mask.operators import object_tool_selection
    from bmanga_dev_layer_mask.ui import context_menu
    from bmanga_dev_layer_mask.utils import layer_stack as layer_stack_utils
    ...
    assert op_id in {"bmanga.layer_detail_open", "bmanga.layer_stack_detail"}, (kind, op_id)
...
def _assert_gp_mask(obj) -> None:
    ...
    for layer in content_layers:
        ...
        assert "__bmanga_mask" in names, f"GPマスク参照がありません: {obj.name}/{layer.name} {names}"


def _assert_no_gp_mask(obj) -> None:
    layers = getattr(getattr(obj, "data", None), "layers", None)
    assert layers is not None, f"GPレイヤーがありません: {obj.name}"
    assert layers.get("__bmanga_mask") is None, f"不要なGPマスクがあります: {obj.name}"
    for layer in layers:
...
def _gp_mask_bounds_mm(obj) -> tuple[float, float, float, float]:
    from bmanga_dev_layer_mask.utils.geom import m_to_mm
...
def main() -> None:
    temp_root = Path(tempfile.mkdtemp(prefix="bmanga_layer_mask_"))
    mod = None
    try:
        bpy.ops.wm.read_factory_settings(use_empty=True)
        mod = _load_addon()
        result = bpy.ops.bmanga.work_new(filepath=str(temp_root / "LayerMask.bmanga"))
        ...
        from bmanga_dev_layer_mask.operators import effect_line_op, raster_layer_op, text_op
        from bmanga_dev_layer_mask.utils import active_target
        from bmanga_dev_layer_mask.utils import balloon_curve_object
        from bmanga_dev_layer_mask.utils import gp_object_layer
        from bmanga_dev_layer_mask.utils import image_real_object
        from bmanga_dev_layer_mask.utils import layer_stack as layer_stack_utils
        from bmanga_dev_layer_mask.utils import mask_apply
        from bmanga_dev_layer_mask.utils import object_selection
        from bmanga_dev_layer_mask.utils import text_real_object
        from bmanga_dev_layer_mask.utils.layer_hierarchy import coma_stack_key, page_stack_key
        ...
        from bmanga_dev_layer_mask.utils import coma_plane
        ...
        assert "FINISHED" in result, result
        raster = scene.bmanga_raster_layers[scene.bmanga_active_raster_layer_index]
...
if __name__ == "__main__":
    main()
    print("BMANGA_LAYER_DETAIL_AND_MASK_OK")
