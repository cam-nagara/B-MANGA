"""Blender実機用: B-Name本体の詳細設定チェックボックスをAI目視用に一覧化する."""
...
OUT_DIR = Path(
    os.environ.get("BNAME_FULL_VISUAL_OUT", "")
    or tempfile.mkdtemp(prefix="bname_full_visual_")
)


def _load_addon():
    spec = importlib.util.spec_from_file_location(
        "bname_dev_full_audit",
        ROOT / "__init__.py",
        submodule_search_locations=[str(ROOT)],
    )
    mod = importlib.util.module_from_spec(spec)
    sys.modules["bname_dev_full_audit"] = mod
...
def _raster_obj_for(entry):
    from bname_dev_full_audit.operators import raster_layer_op
    from bname_dev_full_audit.utils import object_naming as on
...
def _build_scene(context):
    from bname_dev_full_audit.core.work import get_work
    from bname_dev_full_audit.operators import balloon_op
    from bname_dev_full_audit.operators import effect_line_op
    from bname_dev_full_audit.operators import text_op
    from bname_dev_full_audit.utils import gp_layer_parenting
    from bname_dev_full_audit.utils import gpencil as gp_utils
    from bname_dev_full_audit.utils import layer_hierarchy
    from bname_dev_full_audit.utils import object_naming as on
    from bname_dev_full_audit.utils.geom import mm_to_m
    ...
    raster_index = int(context.scene.bname_active_raster_layer_index)
    assert raster_index >= 0
    raster = context.scene.bname_raster_layers[raster_index]
...
def _check_detail_toggles(context, targets) -> list[dict]:
    from bname_dev_full_audit.core import effect_line
    from bname_dev_full_audit.operators import raster_layer_op
    ...
    effect_params = context.scene.bname_effect_line_params
...
def _write_report(results: list[dict]) -> Path:
    ...
    json_path = OUT_DIR / "bname_full_visual_audit.json"
    ...
    draw.text((24, 18), "B-Name 詳細設定チェックボックス AI目視シート", fill=(0, 0, 0), font=title_font)
    ...
    for item in results:
        ...
        y += row_h
    image_path = OUT_DIR / "bname_full_visual_audit.png"
    image.save(image_path)
    return image_path


def main() -> None:
    temp_root = Path(tempfile.mkdtemp(prefix="bname_full_visual_audit_"))
    mod = None
    try:
        bpy.ops.wm.read_factory_settings(use_empty=True)
        mod = _load_addon()
        result = bpy.ops.bname.work_new(filepath=str(temp_root / "FullVisualAudit.bname"))
        assert result == {"FINISHED"}, result
        context = bpy.context
        ...
        print(f"BNAME_FULL_VISUAL_AUDIT_OK visual={report} items={len(results)}")
