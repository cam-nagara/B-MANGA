...
def _setup_camera() -> None:
    bpy.ops.object.camera_add(location=(0.105, 0.105, 1.0), rotation=(0.0, 0.0, 0.0))
...
def main() -> None:
    ...
    display.data.materials.append(_material("白い抜き面", (1.0, 1.0, 1.0, 1.0)))
    display.data.materials.append(_material("白抜き線色", (1.0, 1.0, 1.0, 1.0)))
    ...
    if not any(poly.material_index == 1 for poly in mesh.polygons):
        raise AssertionError("白い抜き面の面が表示メッシュにありません")
    ...
    print(f"BNAME_EFFECT_LINE_WHITE_OUTLINE_VISUAL_OK screenshot={OUT_PATH}", flush=True)
