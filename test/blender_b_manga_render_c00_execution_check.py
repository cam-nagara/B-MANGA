"""Blender実機用: c00.blend 上で B-MANGA Render 全プリセットの実行準備を確認."""
...
def _load_render_package():
    ...
    spec = importlib.util.spec_from_file_location(
        "bmanga_render_c00_exec",
        package_root / "__init__.py",
        submodule_search_locations=[str(package_root)],
    )
    mod = importlib.util.module_from_spec(spec)
    sys.modules["bmanga_render_c00_exec"] = mod
...
def _run_all_presets(context, render_mod) -> dict:
    from bmanga_render_c00_exec import command_runner, core, eevr_bridge
...
def _assert_eevr_bridge(context, temp_root: Path) -> dict:
    from bmanga_render_c00_exec import command_runner, eevr_bridge

    eevr_zip = Path(os.environ.get("BMANGA_EEVR_ZIP", str(DEFAULT_EEVR_ZIP)))
...
def main() -> None:
    ...
    temp_root = Path(tempfile.mkdtemp(prefix="bmanga_render_c00_exec_"))
    render_mod = None
    try:
        ...
        print(
            "BMANGA_RENDER_C00_EXECUTION_OK "
