...
_OUT_DIR_RAW = Path(os.environ.get("BNAME_RENDER_FULL_FIXTURE_OUT", "") or ROOT / ".codex" / "ai_audit" / "bname_render_full_fixture")
OUT_DIR = _OUT_DIR_RAW if _OUT_DIR_RAW.is_absolute() else ROOT / _OUT_DIR_RAW
RESOLUTION = int(os.environ.get("BNAME_RENDER_FULL_FIXTURE_RES", "48"))
SAMPLES = int(os.environ.get("BNAME_RENDER_FULL_FIXTURE_SAMPLES", "1"))
PRESET_REGEX = os.environ.get("BNAME_RENDER_FULL_FIXTURE_PRESET_REGEX", "").strip()
MODE_FILTER = {
    item.strip().lower()
    for item in os.environ.get("BNAME_RENDER_FULL_FIXTURE_MODES", "normal,fisheye").split(",")
    if item.strip()
}
FLOW_ENABLED = os.environ.get("BNAME_RENDER_FULL_FIXTURE_FLOW", "1").strip() != "0"
PREFIX = "BNAME_RENDER_FIXTURE"
RENDER_COMMANDS = {
    "RENDER",
    "RENDER_LAYER",
...
def _load_render_package():
    ...
    spec = importlib.util.spec_from_file_location(
        "bname_render_full_fixture",
        package_root / "__init__.py",
        submodule_search_locations=[str(package_root)],
    )
    mod = importlib.util.module_from_spec(spec)
    sys.modules["bname_render_full_fixture"] = mod
...
def _build_fixture_scene(render_mod) -> Path:
    ...
    fixture_path = OUT_DIR / "b_name_render_full_fixture.blend"
...
def _run_preset(render_mod, preset, index: int, *, fisheye: bool) -> dict:
    from bname_render_full_fixture import command_runner
...
def _run_shared_flow(render_mod, preset_pairs: list[tuple[int, object]]) -> dict:
    ...
    scene = bpy.context.scene
    state = scene.bname_render_state
    ...
    for index, preset in preset_pairs:
        ...
        if "RELOAD_IMAGES" in command_types:
            reloads.append(
                {
                    "preset": preset.name,
                    "candidates": int(scene.get("bname_render_reload_candidate_count", 0) or 0),
                    "matches": int(scene.get("bname_render_reload_match_count", 0) or 0),
                    "images": int(scene.get("bname_render_reload_image_count", 0) or 0),
                }
            )
        if command_types & RENDER_COMMANDS:
            stages.append(
    ...
    flow_contact = _write_flat_sheet(
        stats,
        OUT_DIR / "b_name_render_full_fixture_flow.png",
        "B-Name-Render 共有passes パス→再読み込み→合成 検証",
    )
    composite_contact = _write_flat_sheet(
        composite_stats,
        OUT_DIR / "b_name_render_full_fixture_flow_composites.png",
        "B-Name-Render 共有passes 合成段 AI目視",
    )
...
def _write_category_sheets(results: list[dict]) -> dict[str, str]:
    ...
    return {
        "paths": str(_write_flat_sheet(path_images, OUT_DIR / "b_name_render_full_fixture_paths.png", "B-Name-Render パス画像 AI目視")),
        "composites": str(_write_flat_sheet(composite_images, OUT_DIR / "b_name_render_full_fixture_composites.png", "B-Name-Render コンポジット済み画像 AI目視")),
        "fisheye": str(_write_flat_sheet(fisheye_images, OUT_DIR / "b_name_render_full_fixture_fisheye.png", "B-Name-Render 魚眼画像 AI目視")),
    }
...
def _write_contact_sheet(results: list[dict]) -> Path:
    ...
    draw.text((24, 18), "B-Name-Render 全プリセット fixture 実エンジン/魚眼オン検証", fill=(0, 0, 0), font=title_font)
    ...
    for index, item in enumerate(results, 1):
        ...
        y += row_h
    out_path = OUT_DIR / "b_name_render_full_fixture_contact.png"
    sheet.save(out_path)
    return out_path
...
def main() -> None:
    ...
    try:
        ...
        json_path = OUT_DIR / "b_name_render_full_fixture_results.json"
        ...
        print(f"BNAME_RENDER_FULL_FIXTURE_DONE visual={contact} json={json_path} presets={len(results)} errors={len(failures)}")
