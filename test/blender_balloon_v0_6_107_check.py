"""Blender 5.2 ヘッドレス検証: v0.6.107 の修正:
1. 多重線が角 (とくに谷) でごちゃごちゃせず、意図しないトゲが出ないこと
2. トゲ曲線のピークが直線ではなく曲線になっていること

走らせ方:
  & "C:\\Program Files\\Blender Foundation\\Blender 5.1\\blender.exe" --background --python ^
    "d:/Develop/Blender/B-MANGA/test/blender_balloon_v0_6_107_check.py"
"""
...
_OUT_ENV = os.environ.get("BMANGA_V107_OUT", "")
_OUT_PATH = Path(_OUT_ENV) if _OUT_ENV else Path(tempfile.mkdtemp(prefix="bmanga_v107_"))
...
def _load_addon():
    ...
    sys.modules["bmanga_dev_v107"] = mod
...
def main() -> None:
    temp_root = Path(tempfile.mkdtemp(prefix="bmanga_v107_work_"))
    _OUT_PATH.mkdir(parents=True, exist_ok=True)

    bpy.ops.wm.read_factory_settings(use_empty=True)
    _load_addon()
    result = bpy.ops.bmanga.work_new(filepath=str(temp_root / "V107Check.bmanga"))
    assert "FINISHED" in result, result

    from bmanga_dev_v107.core.work import get_work
    from bmanga_dev_v107.operators import balloon_op
    from bmanga_dev_v107.utils import balloon_curve_object
    from bmanga_dev_v107.utils.layer_hierarchy import page_stack_key
