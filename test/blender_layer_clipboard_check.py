"""Blender 実機用: B-Name レイヤー / フキダシしっぽのコピー貼り付け確認."""
...
def _create_work(work_dir: Path):
    ...
    raster_result = bpy.ops.bname.raster_layer_add(
        "EXEC_DEFAULT",
        dpi=30,
        bit_depth="gray8",
        enter_paint=False,
...
def _copy_paste_tail() -> None:
    ...
    result = bpy.ops.bname.balloon_tail_clipboard_paste("EXEC_DEFAULT")
    assert "FINISHED" in result, result
...
def _assert_shortcuts_registered() -> None:
    ...
    assert has_kmi("bname.layer_clipboard_copy", "C", ctrl=True)
    assert has_kmi("bname.layer_clipboard_paste", "V", ctrl=True)
    assert has_kmi("bname.balloon_tail_clipboard_copy", "C", ctrl=True, shift=True)
    assert has_kmi("bname.balloon_tail_clipboard_paste", "V", ctrl=True, shift=True)


def main() -> None:
    temp_root = Path(tempfile.mkdtemp(prefix="bname_layer_clipboard_"))
    mod = None
    try:
        bpy.ops.wm.read_factory_settings(use_empty=True)
        mod = _load_addon()
        _create_work(temp_root / "Layer_Clipboard.bname")
        for kind in ("balloon", "text", "raster", "effect", "gp"):
            _copy_paste_kind(kind)
        _copy_paste_tail()
        _assert_shortcuts_registered()
        print("BNAME_LAYER_CLIPBOARD_OK")
    finally:
        if mod is not None:
            try:
                mod.unregister()
