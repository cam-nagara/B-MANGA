"""v0.6.128 検証: ユーザー test101 設定 (山の線幅 0% で cut endpoint も 0% に収束).

走らせ方:
  & "C:\\Program Files\\Blender Foundation\\Blender 5.1\\blender.exe" --background --python ^
    "d:/Develop/Blender/B-MANGA/test/blender_balloon_v0_6_128_check.py"
"""
...
_OUT_ENV = os.environ.get("BMANGA_V128_OUT", "")
_OUT_PATH = Path(_OUT_ENV) if _OUT_ENV else Path(tempfile.mkdtemp(prefix="bmanga_v128_"))
...
def _load_addon():
    ...
    sys.modules["bmanga_dev_v128"] = mod
...
def main() -> None:
    temp_root = Path(tempfile.mkdtemp(prefix="bmanga_v128_work_"))
    _OUT_PATH.mkdir(parents=True, exist_ok=True)
    bpy.ops.wm.read_factory_settings(use_empty=True)
    _load_addon()
    result = bpy.ops.bmanga.work_new(filepath=str(temp_root / "V128Check.bmanga"))
    assert "FINISHED" in result, result

    from bmanga_dev_v128.core.work import get_work
    from bmanga_dev_v128.operators import balloon_op
    from bmanga_dev_v128.utils import balloon_curve_object
    from bmanga_dev_v128.utils.layer_hierarchy import page_stack_key
    ...
    # ユーザー test101 完全再現:
    # 形状: トゲ直線, 山の幅 12.64 / 山の高さ 19.15 / 小山幅 40.83 / 小山高 85.29
    # 線本数 4 外側 / 多重線幅 1.44 / 多重線間隔 0.40
    # 長さ 88.30 / 27.66 / 山谷を延ばして交差 ON
    # 谷の線幅 100 / 山の線幅 0 (← 重要!)
    entry = balloon_op._create_balloon_entry(
        context, page,
        shape="thorn", x=30.0, y=30.0, w=49.50, h=54.49,
        parent_kind="page", parent_key=parent_key,
    )
    entry.line_style = "double"
    entry.line_width_mm = 1.47
    ...
    entry.multi_line_width_mm = 1.44
    ...
    entry.thorn_multi_line_peak_width_pct = 0.0  # ← 山の線幅 0%
    entry.thorn_multi_line_length_scale_near_percent = 88.30
    entry.thorn_multi_line_length_scale_far_percent = 27.66
    ...
    sp.cloud_bump_width_mm = 12.64
    sp.cloud_bump_width_jitter = 0.00
    sp.cloud_bump_height_mm = 19.15
    sp.cloud_bump_height_jitter = 0.00
    sp.cloud_offset_percent = 50.0
    sp.cloud_sub_width_ratio = 40.83
    sp.cloud_sub_height_ratio = 85.29
    sp.shape_seed = 0
    obj = balloon_curve_object.ensure_balloon_curve_object(scene=context.scene, entry=entry, page=page)
    bx = float(obj.location.x)
    by = float(obj.location.y)
    _set_ortho_camera(bx, by, 0.20)
    out = _OUT_PATH / "13_test101.png"
    _render_to(out)
    print(f"[OUT] test101 (山の線幅 0% + length cut): {out}")
