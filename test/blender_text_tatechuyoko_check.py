...
反映されない」問題への対応:
  ...
  2. typography/metrics.py へ auto_tatechuyoko_ranges() を追加。Meldex と同じく
     空白以外の printable ASCII を、記号・文字数を限定せず自動検出する。
...
検証項目:
  ...
  2. 長いラン・単独文字・ASCII 記号も変換され、空白・改行・全角で分割される
...
def _run_check() -> None:
    ...
    try:
        ...
        # --- 2. Meldex と同じく文字数制限なし + ASCII 記号を余さず対象にする ---
        text2 = "no1234567"
        auto_ranges2 = metrics.auto_tatechuyoko_ranges(text2)
        _check(auto_ranges2 == [(0, len(text2))], f"長いランが縦中横候補になっていません: {auto_ranges2!r}")
        result2 = layout.typeset_vertical(
            text2,
            0.0,
            0.0,
            60.0,
            200.0,
            font_size_pt=geom.q_to_pt(20.0),
            tatechuyoko_ranges=auto_ranges2,
        )
        ...
        _check(len(ys2) == 1, f"長いランが1つの縦中横セルへ圧縮されていません: {ys2!r}")
        sizes2 = {round(g.size_pt, 6) for g in by2.values()}
        _check(
            sizes2 == {round(geom.q_to_pt(20.0) * 2.0 / len(text2), 6)},
            f"長いランが1emへ収まるサイズに縮小されていません: {sizes2!r}",
        )
        ascii_printable = "".join(chr(code) for code in range(0x21, 0x7F))
        _check(
            metrics.auto_tatechuyoko_ranges(ascii_printable) == [(0, len(ascii_printable))],
            "空白以外の printable ASCII 全体が1つの縦中横候補になっていません",
        )
        _check(metrics.auto_tatechuyoko_ranges("A") == [(0, 1)], "ASCII 1文字が候補になっていません")
        _check(
            metrics.auto_tatechuyoko_ranges("AB CD\nＥＦ") == [(0, 2), (3, 2)],
            "空白・改行・全角文字でASCIIランが正しく分割されません",
        )
        _check(metrics.is_tatechuyoko_candidate("/\\|:_=+-"), "主要ASCII記号が候補になりません")
        _check(not metrics.is_tatechuyoko_candidate("A B"), "空白を含む文字列が候補になっています")
