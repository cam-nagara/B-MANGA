...
OUT_DIR = Path(os.environ.get("BMANGA_RENDER_RANGE_OUT", "") or ROOT / ".codex" / "ai_audit" / "bmanga_render_c00_range")
...
def _ensure_camera(scene):
    if scene.camera is not None and scene.camera.type == "CAMERA":
        return scene.camera
    cam_data = bpy.data.cameras.new("B-MANGA Render範囲監査カメラ")
    cam = bpy.data.objects.new("B-MANGA Render範囲監査カメラ", cam_data)
...
def _add_background(camera, image_name: str, *, managed: bool, scale: float, width: int, height: int):
    ...
    if managed:
        ...
        image["bmanga_page_id"] = "p0001"
        image["bmanga_coma_id"] = "c00"
        image["bmanga_full_page_mask"] = False
...
def _write_range_sheet(evidence: dict, path: Path) -> Path:
    ...
    cases = [
        ...
        ("B-MANGA Render後", evidence["after_b_manga_render"]),
    ]
    sheet = Image.new("RGB", (1120, 420), "white")
    draw = ImageDraw.Draw(sheet)
    title_font = font(20)
    body_font = font(13)
    draw.text((24, 18), "B-MANGA Render c00 出力範囲 / 魚眼 / 縮小 / 再オープン検証", fill=(0, 0, 0), font=title_font)
...
def _run_render_preset_without_pixels(context, render_mod, preset_name: str) -> int:
    from bmanga_render_range import command_runner, eevr_bridge
...
def main() -> None:
    ...
    try:
        ...
        bmanga = _load_package("bmanga_dev_range", ROOT)
        render = _load_package("bmanga_render_range", ROOT / "addons" / "b_manga_render")
        scene = bpy.context.scene
        camera = _ensure_camera(scene)
        scene.bmanga_current_coma_page_id = "p0001"
        scene.bmanga_current_coma_id = "c00"
        scene.render.resolution_x = 6071
        scene.render.resolution_y = 8598
        scene.bmanga_coma_camera_original_resolution_x = 6071
        scene.bmanga_coma_camera_original_resolution_y = 8598
        ...
        managed_bg = _add_background(camera, "BManga_コマ_p0001_c00_page", managed=True, scale=0.75, width=1200, height=600)
        assert camera.data.background_images[-2].image == old_bg.image
        assert camera.data.background_images[-1].image == managed_bg.image

        from bmanga_dev_range.utils import coma_camera

        coma_camera.update_render_border_from_current_coma(bpy.context)
        normal = {
            "resolution": [scene.render.resolution_x, scene.render.resolution_y],
            "border": _border(scene),
            "source": scene.get("bmanga_coma_camera_render_border_source", ""),
        }
        assert normal["source"] == "BManga_コマ_p0001_c00_page", normal
        ...
        scene.bmanga_coma_camera_fisheye_fov = 3.1415927
        ...
        copy_path = temp_root / "bmanga_render_output_range_roundtrip.blend"
        bpy.ops.wm.save_as_mainfile(filepath=str(copy_path))
        reopen_json = temp_root / "bmanga_render_output_range_reopen.json"
        env = os.environ.copy()
        env["BMANGA_RANGE_REOPEN_BLEND"] = str(copy_path)
        env["BMANGA_RANGE_REOPEN_JSON"] = str(reopen_json)
        subprocess.run(
            [
                bpy.app.binary_path,
                "--factory-startup",
                "--background",
                "--python",
                str(ROOT / "test" / "blender_b_manga_render_c00_output_range_reopen_worker.py"),
            ],
            check=True,
            env=env,
        )
        ...
        evidence = {
            ...
            "after_b_manga_render": after_preset,
        }
        out_path = temp_root / "bmanga_render_output_range_roundtrip.json"
        evidence["visual_sheet"] = str(_write_range_sheet(evidence, temp_root / "bmanga_render_output_range_roundtrip.png"))
        out_path.write_text(json.dumps(evidence, ensure_ascii=False, indent=2), encoding="utf-8")
        print(f"BMANGA_RENDER_C00_OUTPUT_RANGE_OK {out_path}")
    finally:
