...
def main() -> None:
    ...
    try:
        ...
        work_records = _draw_records(work_panel.BNAME_PT_work, context)
        _assert_present(work_records, "ページ一覧に戻る", "作品情報")
        _assert_absent(work_records, "ページ数", "ページ一覧プレビュー", "コマ用blendファイル (この作品のみ)")
        view_records = _draw_records(view_panel.BNAME_PT_view, context)
        _assert_present(view_records, "ページ一覧表示", "前後ページ数", "画像解像度%")
        _assert_absent(view_records, "全ページを一覧", "選択ページ", "列数", "間隔mm")
        layer_records = _draw_records(gpencil_panel.BNAME_PT_layer_stack, context)
