"""Blender 5.2 ヘッドレス検証: v0.6.110 の修正:
1. 「間隔変化 (%)」 が機能する (新規)
2. 「長さ変化 (%)」「谷の線幅」「山の線幅」 が cloud/fluffy/thorn/thorn-curve で機能する

走らせ方:
  & "C:\\Program Files\\Blender Foundation\\Blender 5.1\\blender.exe" --background --python ^
    "d:/Develop/Blender/B-MANGA/test/blender_balloon_v0_6_110_check.py"
"""
...
_OUT_ENV = os.environ.get("BMANGA_V110_OUT", "")
_OUT_PATH = Path(_OUT_ENV) if _OUT_ENV else Path(tempfile.mkdtemp(prefix="bmanga_v110_"))
...
def _load_addon():
    ...
    sys.modules["bmanga_dev_v110"] = mod
...
def main() -> None:
    temp_root = Path(tempfile.mkdtemp(prefix="bmanga_v110_work_"))
    _OUT_PATH.mkdir(parents=True, exist_ok=True)

    bpy.ops.wm.read_factory_settings(use_empty=True)
    _load_addon()
    result = bpy.ops.bmanga.work_new(filepath=str(temp_root / "V110Check.bmanga"))
    assert "FINISHED" in result, result

    from bmanga_dev_v110.core.work import get_work
    from bmanga_dev_v110.operators import balloon_op
    from bmanga_dev_v110.utils import balloon_curve_object
    from bmanga_dev_v110.utils.layer_hierarchy import page_stack_key
