"""v0.6.124 検証: ユーザー test98 設定そのままで cross スパイクが適切な大きさになるか.

走らせ方:
  & "C:\\Program Files\\Blender Foundation\\Blender 5.1\\blender.exe" --background --python ^
    "d:/Develop/Blender/B-MANGA/test/blender_balloon_v0_6_124_check.py"
"""
...
_OUT_ENV = os.environ.get("BMANGA_V124_OUT", "")
_OUT_PATH = Path(_OUT_ENV) if _OUT_ENV else Path(tempfile.mkdtemp(prefix="bmanga_v124_"))
...
def _load_addon():
    ...
    sys.modules["bmanga_dev_v124"] = mod
...
def main() -> None:
    temp_root = Path(tempfile.mkdtemp(prefix="bmanga_v124_work_"))
    _OUT_PATH.mkdir(parents=True, exist_ok=True)
    bpy.ops.wm.read_factory_settings(use_empty=True)
    _load_addon()
    result = bpy.ops.bmanga.work_new(filepath=str(temp_root / "V124Check.bmanga"))
    assert "FINISHED" in result, result

    from bmanga_dev_v124.core.work import get_work
    from bmanga_dev_v124.operators import balloon_op
    from bmanga_dev_v124.utils import balloon_curve_object
    from bmanga_dev_v124.utils.layer_hierarchy import page_stack_key
    ...
    entry.thorn_multi_line_length_scale_far_percent = 57.45
    entry.thorn_multi_line_cross_enabled = True
    sp = entry.shape_params
    sp.cloud_valley_sharp = True
    sp.cloud_bump_width_mm = 13.51
    sp.cloud_bump_height_mm = 18.52
    sp.cloud_offset_percent = 50.0
    sp.cloud_sub_width_ratio = 0.0
    sp.cloud_sub_height_ratio = 0.0
    obj = balloon_curve_object.ensure_balloon_curve_object(scene=context.scene, entry=entry, page=page)

    # camera を balloon 全体が見えるよう設定 (体周囲 30mm 余裕)
    bx = float(obj.location.x)
    by = float(obj.location.y)
    _set_ortho_camera(bx, by, 0.20)  # 200mm 視野で 80mm 体を表示
    out = _OUT_PATH / "08_user_test98.png"
    _render_to(out)
    print(f"[OUT] user test98 (line_width 3.48, cross ON, length 100/57.45): {out}")

    # cross OFF 版も比較用に
    entry.thorn_multi_line_cross_enabled = False
    obj = balloon_curve_object.ensure_balloon_curve_object(scene=context.scene, entry=entry, page=page)
    out2 = _OUT_PATH / "09_user_test98_no_cross.png"
    _render_to(out2)
    print(f"[OUT] user test98 (cross OFF for comparison): {out2}")
