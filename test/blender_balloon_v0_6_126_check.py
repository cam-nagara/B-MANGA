...
走らせ方:
  & "C:\\Program Files\\Blender Foundation\\Blender 5.1\\blender.exe" --background --python ^
    "d:/Develop/Blender/B-Name/test/blender_balloon_v0_6_126_check.py"
"""
...
_OUT_ENV = os.environ.get("BNAME_V126_OUT", "")
_OUT_PATH = Path(_OUT_ENV) if _OUT_ENV else Path(tempfile.mkdtemp(prefix="bname_v126_"))
...
def _load_addon():
    ...
    sys.modules["bname_dev_v126"] = mod
...
def main() -> None:
    temp_root = Path(tempfile.mkdtemp(prefix="bname_v126_work_"))
    _OUT_PATH.mkdir(parents=True, exist_ok=True)
    bpy.ops.wm.read_factory_settings(use_empty=True)
    _load_addon()
    result = bpy.ops.bname.work_new(filepath=str(temp_root / "V126Check.bname"))
    assert "FINISHED" in result, result

    from bname_dev_v126.core.work import get_work
    from bname_dev_v126.operators import balloon_op
    from bname_dev_v126.utils import balloon_curve_object
    from bname_dev_v126.utils.layer_hierarchy import page_stack_key
