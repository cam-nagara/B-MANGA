"""v0.6.126 検証: anchor ベース peak 検出 + jittered 高さ + 主線可変幅.

走らせ方:
  & "C:\\Program Files\\Blender Foundation\\Blender 5.1\\blender.exe" --background --python ^
    "d:/Develop/Blender/B-MANGA/test/blender_balloon_v0_6_126_check.py"
"""
...
_OUT_ENV = os.environ.get("BMANGA_V126_OUT", "")
_OUT_PATH = Path(_OUT_ENV) if _OUT_ENV else Path(tempfile.mkdtemp(prefix="bmanga_v126_"))
...
def _load_addon():
    ...
    sys.modules["bmanga_dev_v126"] = mod
...
def main() -> None:
    temp_root = Path(tempfile.mkdtemp(prefix="bmanga_v126_work_"))
    _OUT_PATH.mkdir(parents=True, exist_ok=True)
    bpy.ops.wm.read_factory_settings(use_empty=True)
    _load_addon()
    result = bpy.ops.bmanga.work_new(filepath=str(temp_root / "V126Check.bmanga"))
    assert "FINISHED" in result, result

    from bmanga_dev_v126.core.work import get_work
    from bmanga_dev_v126.operators import balloon_op
    from bmanga_dev_v126.utils import balloon_curve_object
    from bmanga_dev_v126.utils.layer_hierarchy import page_stack_key
    ...
    # === 1) test100 case 1: jittered 高さ + cross ON + 長さ 90/22 ===
    entry = balloon_op._create_balloon_entry(
        context, page,
        shape="thorn", x=30.0, y=30.0, w=50.58, h=54.71,
    ...
    out = _OUT_PATH / "11_test100_case1.png"
    _render_to(out)
    print(f"[OUT] test100 case 1 (jittered, cross ON, length 90/22): {out}")

    # === 2) test100 case 2: 主線・山の線幅 12.23% (= 主線が細くなる) ===
    entry.line_peak_width_pct = 12.23
    obj = balloon_curve_object.ensure_balloon_curve_object(scene=context.scene, entry=entry, page=page)
    out2 = _OUT_PATH / "12_test100_case2_peak_width_12_23.png"
    _render_to(out2)
    print(f"[OUT] test100 case 2 (peak_width 12.23% - main line corners): {out2}")
