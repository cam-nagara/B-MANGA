"""Blender実機用: B-MANGA が c00.blend をコマ用テンプレートとして使えることを確認."""
...
def _load_addon():
    spec = importlib.util.spec_from_file_location(
        "bmanga_dev_c00_template",
        ROOT / "__init__.py",
        submodule_search_locations=[str(ROOT)],
    )
    mod = importlib.util.module_from_spec(spec)
    sys.modules["bmanga_dev_c00_template"] = mod
...
def _assert_bmanga_coma_state(work_dir: Path) -> dict:
    from bmanga_dev_c00_template.core.mode import MODE_COMA, get_mode
    from bmanga_dev_c00_template.utils.coma_camera_constants import MANAGED_IMAGE_PROP
    from bmanga_dev_c00_template.utils.geom import mm_to_px
    ...
    for bg in getattr(data, "background_images", []) or []:
        ...
        try:
            ...
            pass
    assert managed, "B-MANGA page image background was not added to c00 camera"
...
def main() -> None:
    ...
    temp_root = Path(tempfile.mkdtemp(prefix="bmanga_c00_template_"))
    work_dir = temp_root / "C00_Template.bmanga"
    mod = None
    try:
        ...
        bmanga_result = _assert_bmanga_coma_state(work_dir)
        ...
        reopen_bmanga_result = _assert_bmanga_coma_state(work_dir)
        print(
            "BMANGA_C00_TEMPLATE_INTEGRATION_OK "
            f"template={template_result} bmanga={bmanga_result} "
            f"reopen={reopen_bmanga_result}"
        )
    finally:
        if mod is not None:
            try:
