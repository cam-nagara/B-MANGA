...
def main() -> None:
    mod = _load_addon()
    temp_root = Path(tempfile.mkdtemp(prefix="bmanga_restructure_"))
    work_dir = temp_root / "R5_Test.bmanga"
    try:
        ...
        result = bpy.ops.bmanga.coma_move_to_page(target_page_id="p0001")
        assert result == {"FINISHED"}, result
        ...
        result = bpy.ops.bmanga.pages_split_spread(spread_index=0)
        assert result == {"FINISHED"}, result
        assert (work_dir / "p0001" / "page.json").exists()
        assert (work_dir / "p0002" / "page.json").exists()
