"""Blender実機用: B-Name-Render UI導線とカード設定の監査."""
...
def _load_render_package():
    ...
    spec = importlib.util.spec_from_file_location(
        "bname_render_ui_audit",
        package_root / "__init__.py",
        submodule_search_locations=[str(package_root)],
    )
    mod = importlib.util.module_from_spec(spec)
    sys.modules["bname_render_ui_audit"] = mod
...
def _assert_command_ui(mod) -> dict:
    from bname_render_ui_audit import command_ui, core
...
def _assert_panel_access(mod) -> dict:
    ...
    assert getattr(bpy.types, "BNAME_RENDER_PT_node", None) is not None
    main = bpy.types.BNAME_RENDER_PT_main
    node = bpy.types.BNAME_RENDER_PT_node
    assert main.bl_space_type == "VIEW_3D"
    assert main.bl_region_type == "UI"
    assert main.bl_category == "B-Name-Render"
    assert node.bl_space_type == "NODE_EDITOR"
    assert node.bl_region_type == "UI"
    assert node.bl_category == "B-Name-Render"
...
def _make_visual_sheet(access: dict, command_results: dict, resolution: dict) -> str:
    ...
    y = 40
    text(24, y, "B-Name-Render UI / アクションリスト 仕様監査", 24, "700", "#000")
    y += 44
    ...
    for identifier, data in command_results.items():
        ...
        y += row_h
    lines.append("</svg>")
    path = OUT_DIR / "b_name_render_ui_audit.svg"
...
def main() -> None:
    ...
    try:
        ...
        result_path = OUT_DIR / "b_name_render_ui_audit.json"
        ...
        print(f"BNAME_RENDER_UI_AUDIT_OK visual={visual_path}")
    finally:
        if mod is not None:
            try:
                mod.unregister()
