...
def create_visual_rig(scene) -> list[bpy.types.Object]:
    ...
    center = camera.location + forward.normalized() * 4.0
    material = bpy.data.materials.new("BML_AUDIT_surface")
    material.diffuse_color = (0.85, 0.85, 0.82, 1.0)
    objects = []
    for index, offset in enumerate((-1.2, 0.0, 1.2)):
        bpy.ops.mesh.primitive_cube_add(size=0.85, location=center + right * offset + up * 0.1)
        obj = bpy.context.object
        obj.name = f"BML_AUDIT_visual_cube_{index}"
        obj.data.materials.append(material)
        objects.append(obj)
    objects[1].rotation_euler[2] = math.radians(32.0)
    objects[2].rotation_euler[1] = math.radians(18.0)
    return objects
...
def phase4(audit: Audit, _max_targets: int) -> None:
    ...
    settings = active_settings()
    with forbid_auto_targets(audit):
        set_line_flags(settings, True, False, False)
        settings.outline_thickness_mm = 1.5
        settings.edge_smooth_factor = 1.0
        settings.edge_midpoint_jitter_percent = 0.0
        apply_selected(audit, targets, "toggle")
        save_camera_render(audit, scene, "outline_midpoint_smooth")

        set_line_flags(settings, False, True, False)
        settings.inner_line_angle = math.radians(20.0)
        settings.inner_line_thickness_mm = 1.2
        settings.inner_edge_smooth_factor = 1.0
        settings.inner_edge_midpoint_jitter_percent = 0.0
        apply_selected(audit, targets, "toggle")
        save_camera_render(audit, scene, "inner_midpoint_smooth")

        set_line_flags(settings, False, False, True)
        settings.intersection_thickness_mm = 1.5
        settings.intersection_edge_smooth_factor = 1.0
        settings.intersection_edge_midpoint_jitter_percent = 0.0
        apply_selected(audit, targets, "toggle")
        save_camera_render(audit, scene, "intersection_midpoint_smooth")
        ...
        apply_selected(audit, targets, "toggle")
        save_camera_render(audit, scene, "midpoint_jitter_all")
