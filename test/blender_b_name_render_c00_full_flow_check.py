"""Blender実機用: c00.blend の実ノードを使った B-Name-Render 完全連動監査."""
...
def _load_render_package():
    ...
    spec = importlib.util.spec_from_file_location(
        "bname_render_full_flow",
        package_root / "__init__.py",
        submodule_search_locations=[str(package_root)],
    )
    mod = importlib.util.module_from_spec(spec)
    sys.modules["bname_render_full_flow"] = mod
...
def _add_probe_mesh(collection_name: str, material: bpy.types.Material, index: int) -> None:
    ...
    mesh = bpy.data.meshes.new(f"BNAME_RENDER_FULL_FLOW_MESH_{collection_name}")
    size = 0.35
    x = (index % 4 - 1.5) * 0.9
    z = (index // 4) * 0.5
    mesh.from_pydata(
    ...
    mesh.update()
    obj = bpy.data.objects.new(f"BNAME_RENDER_FULL_FLOW_{collection_name}", mesh)
...
def _prepare_probe_scene(scene) -> None:
    ...
    if scene.camera is None:
        cam_data = bpy.data.cameras.new("B-Name-Render完全連動監査カメラ")
        cam = bpy.data.objects.new("B-Name-Render完全連動監査カメラ", cam_data)
...
def main() -> None:
    ...
    temp_root = Path(tempfile.mkdtemp(prefix="bname_render_full_flow_"))
    render = None
    try:
        ...
        out_dir = Path(os.environ.get("BNAME_RENDER_FULL_FLOW_OUT", "")) if os.environ.get("BNAME_RENDER_FULL_FLOW_OUT") else temp_root
        out_dir.mkdir(parents=True, exist_ok=True)
        out_path = out_dir / "b_name_render_c00_full_flow.json"
        out_path.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
        print(f"BNAME_RENDER_C00_FULL_FLOW_OK {out_path}")
    finally:
        ...
        if not os.environ.get("BNAME_RENDER_FULL_FLOW_OUT"):
