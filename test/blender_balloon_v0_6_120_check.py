...
走らせ方:
  & "C:\\Program Files\\Blender Foundation\\Blender 5.1\\blender.exe" --background --python ^
    "d:/Develop/Blender/B-MANGA/test/blender_balloon_v0_6_120_check.py"
"""
...
_OUT_ENV = os.environ.get("BMANGA_V120_OUT", "")
_OUT_PATH = Path(_OUT_ENV) if _OUT_ENV else Path(tempfile.mkdtemp(prefix="bmanga_v120_"))
...
def _load_addon():
    ...
    sys.modules["bmanga_dev_v120"] = mod
...
def main() -> None:
    temp_root = Path(tempfile.mkdtemp(prefix="bmanga_v120_work_"))
    ...
    # --- 7) ユーザー test97 再現: 線本数 6, 谷=72.87%, 山=38.83%, 長さ 100/55.32, cross ON ---
    _delete_all_balloons(page)
    balloon_curve_object.cleanup_orphan_balloon_objects(context.scene)
    entry = balloon_op._create_balloon_entry(
        context, page,
    ...
    _center_and_render([obj], out)
    print(f"[OUT] user test97 (6 rings, cross ON, non-uniform width): {out}")
