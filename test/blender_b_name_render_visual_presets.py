"""Blender実機用: B-Name-Render全プリセットの実レンダーをAI目視用に一覧化する."""
...
OUT_DIR = Path(
    os.environ.get("BNAME_RENDER_VISUAL_OUT", "")
    or (ROOT / ".codex" / "visual" / "bname_render_presets")
)
RESOLUTION = int(os.environ.get("BNAME_RENDER_VISUAL_RES", "96"))
SAMPLE_OVERRIDE = int(os.environ.get("BNAME_RENDER_VISUAL_SAMPLES", "1"))
RENDER_MODE = os.environ.get("BNAME_RENDER_VISUAL_MODE", "RENDER").upper()
FORCE_ENGINE = os.environ.get("BNAME_RENDER_VISUAL_FORCE_ENGINE", "BLENDER_WORKBENCH")
PROXY_COLLECTIONS = (
...
def _load_render_package():
    ...
    sys.modules["bname_render_visual"] = mod
...
def _ensure_camera(scene) -> None:
    if scene.camera is not None:
        return
    cam_data = bpy.data.cameras.new("B-Name-Render監査カメラ")
    cam = bpy.data.objects.new("B-Name-Render監査カメラ", cam_data)
...
def _create_proxy_scene(preset_library):
    ...
    scene = bpy.data.scenes.new("BNameRenderVisualProxy")
...
def _ensure_visual_proxies(scene) -> None:
    if bpy.data.objects.get("BNAME_RENDER_AUDIT_PROXY_レイアウト") is not None:
        return
    light_data = bpy.data.lights.new("BNAME_RENDER_AUDIT_LIGHT", "AREA")
    light = bpy.data.objects.new("BNAME_RENDER_AUDIT_LIGHT", light_data)
    ...
    for index, (collection_name, color) in enumerate(PROXY_COLLECTIONS):
        ...
        if coll is None:
            continue
        mat = bpy.data.materials.new(f"BNAME_RENDER_AUDIT_MAT_{collection_name}")
        mat.diffuse_color = color
        x = (index % 5 - 2) * 1.25
        y = (index // 5 - 1) * 1.05
        mesh = bpy.data.meshes.new(f"BNAME_RENDER_AUDIT_MESH_{collection_name}")
        size = 0.46
        verts = [
            (-size, -size, -size),
            (size, -size, -size),
        ...
        mesh.update()
        obj = bpy.data.objects.new(f"BNAME_RENDER_AUDIT_PROXY_{collection_name}", mesh)
        obj.location = (x, y, 0.0)
        obj.data.materials.append(mat)
        coll.objects.link(obj)
        text_curve = bpy.data.curves.new(f"BNAME_RENDER_AUDIT_TEXT_{collection_name}", "FONT")
        text_curve.body = collection_name
        text_curve.align_x = "CENTER"
        text_curve.size = 0.18
        text = bpy.data.objects.new(f"BNAME_RENDER_AUDIT_LABEL_{collection_name}", text_curve)
...
def _check_ui_toggles(context) -> list[dict]:
    from bname_render_visual import core
    ...
    if state is not None:
        flip("B-Name-Render / 出力完了時アラーム再生", state, "sound_enabled")
...
def _write_contact_sheet(results: list[dict], toggle_checks: list[dict]) -> Path:
    ...
    json_path = OUT_DIR / "b_name_render_visual_presets.json"
    ...
    draw.text((24, 18), "B-Name-Render 全出力プリセット 実レンダー AI目視シート", fill=(0, 0, 0), font=title_font)
    ...
    for index, item in enumerate(results):
        ...
        y += row_h
    image_path = OUT_DIR / "b_name_render_visual_presets.png"
...
def main() -> None:
    ...
    try:
        ...
        state = proxy_scene.bname_render_state
        ...
        print(f"BNAME_RENDER_VISUAL_PRESETS_DONE visual={sheet} presets={len(results)} errors={len(errors)}")
