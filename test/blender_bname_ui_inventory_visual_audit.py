"""Blender実機用: B-Name UI項目の実描画棚卸しをAI目視用画像にする."""
...
OUT_DIR = Path(
    os.environ.get("BNAME_UI_INVENTORY_OUT", "")
    or tempfile.mkdtemp(prefix="bname_ui_inventory_")
)


def _load_addon():
    spec = importlib.util.spec_from_file_location(
        "bname_dev_ui_inventory",
        ROOT / "__init__.py",
        submodule_search_locations=[str(ROOT)],
    )
    mod = importlib.util.module_from_spec(spec)
    sys.modules["bname_dev_ui_inventory"] = mod
...
def _raster_obj_for(entry):
    from bname_dev_ui_inventory.operators import raster_layer_op
    from bname_dev_ui_inventory.utils import object_naming as on
...
def _build_scene(context):
    from bname_dev_ui_inventory.operators import balloon_op, effect_line_op, text_op
    from bname_dev_ui_inventory.utils import gp_layer_parenting
    from bname_dev_ui_inventory.utils import gpencil as gp_utils
    from bname_dev_ui_inventory.utils import layer_hierarchy, layer_stack
    from bname_dev_ui_inventory.utils import object_naming as on
    from bname_dev_ui_inventory.utils.geom import mm_to_m
...
def _registered_panel_classes():
    ...
    return (
        work_panel.BNAME_PT_work,
        work_panel.BNAME_PT_coma_return,
        paper_panel.BNAME_PT_paper,
        tool_panel.BNAME_PT_tools,
        view_panel.BNAME_PT_view,
        gpencil_panel.BNAME_PT_layer_stack,
...
def _collect_panels(records: list[dict[str, Any]], context) -> None:
    ...
    try:
        ...
        _collect_panel(records, context, work_panel.BNAME_PT_coma_return, "B-Nameパネル / コマ編集", force=True)
        _collect_panel(records, context, coma_camera_panel.BNAME_PT_coma_camera, "B-Nameパネル / コマ編集", force=True)
    finally:
...
def _collect_layer_stack_details(records: list[dict[str, Any]], context, targets) -> None:
    from bname_dev_ui_inventory.panels import gpencil_panel
...
def _collect_right_click_details(records: list[dict[str, Any]], context, targets) -> None:
    from bname_dev_ui_inventory.operators import layer_detail_op
...
def _write_json(records: list[dict[str, Any]], missing: list[str]) -> Path:
    ...
    path = OUT_DIR / "bname_ui_inventory_visual_audit.json"
    payload = {
        "records": records,
...
def _write_image(records: list[dict[str, Any]], missing: list[str]) -> Path | None:
    ...
    draw.text((24, 18), "B-Name UI項目 実描画棚卸し AI目視シート", fill=(0, 0, 0), font=title_font)
    draw.text(
        (24, 52),
        f"対象: B-Nameパネル / レイヤー詳細 / 右クリック詳細設定 / 統合済み旧パネル  項目数: {len(records)}",
        fill=(0, 0, 0),
        font=font,
    )
    ...
    path = OUT_DIR / "bname_ui_inventory_visual_audit.png"
    image.save(path)
    return path


def main() -> None:
    temp_root = Path(tempfile.mkdtemp(prefix="bname_ui_inventory_"))
    mod = None
    try:
        bpy.ops.wm.read_factory_settings(use_empty=True)
        mod = _load_addon()
        result = bpy.ops.bname.work_new(filepath=str(temp_root / "UiInventory.bname"))
        assert result == {"FINISHED"}, result
        context = bpy.context
        ...
        print(
            "BNAME_UI_INVENTORY_VISUAL_AUDIT_OK "
