...
Blender 内から実行する::
    ...
    blender --background --python test/blender_b_manga_render_batch_runner_check.py
"""
...
def _compositor_tree(scene):
    ...
    if hasattr(scene, "compositing_node_group"):
        ...
        if tree is None:
            tree = bpy.data.node_groups.new("BMangaBatchComp", "CompositorNodeTree")
...
def _build_scene(out_folder: str) -> None:
    ...
    _make_preset(scene.bmanga_render_state, "fixture", out_folder)
...
def _run() -> int:
    ...
    work = Path(tempfile.mkdtemp(prefix="bmanga_runner_"))
